...
│   ├── version-control/          #   reading a change, staging, committing
...
`B`, terminal `C`, monitoring `D`, agents `E`, coordination `G`, shell `I`, orchestrator `O`, version
control `VC`) — but named
...
| Addressed agent messaging | `orchestration` | orch-04 (O13/O15/O16) | A default `spawn_agent` receives onboarding after idle while an explicit opt-out does not; prompt Tasks, no-todo direct/group messages, list/get/ack, and the optional durable todo completion cross real PTYs and authenticated IPC | `plan/orchestrator/orch-04` | ✅ **covered** — `specs/orchestration/agent-messaging.spec.ts` uses two long-lived fixture workers. The primary's request serializes the default-true setting without an `include_agent_instructions` field and prints proof only after the briefing arrives as a submitted PTY turn; the peer explicitly sends `false` and fails if the briefing appears. Their direct/group exchange deliberately carries no todo correlation; only the final durable board/result path uses `agent_report_completion`. The fixture stays alive after each proof so a fast scenario cannot race the sidebar's `Running` observation. **What the walk reads is each agent's own terminal output** — the app renders no message bodies anywhere, so every assertion is evidence the core carried the message into the recipient *process*, never that a window surface displayed one; the bodies are legible only because the fixture echoes what it received, which no real agent CLI does. Both workers are PATH stubs. **The lead fixture must set an OSC title**, which the first run is what caught: it stands in for a title-status provider, and an agent whose provider signal has never appeared is not classified at all — so a title-less lead is never idle and the idle-gated wake carrying its completion notice never reaches it (the timers walk passes with the same binary because the scheduler's PTY delivery is not idle-gated). Mutation-verified twice: flipping the wire default for `include_agent_instructions`, and broadcasting to nobody, each reddens **only** this spec file |
| Scratchpad & to-do panels | `coordination` | orch-02 (O5/O6/O12/O14) | Edit + save a scratchpad and force a revision conflict; a blocker chain refuses complete then allows it; a comment renders its author; "Copy link" puts the `solo://` URL on the clipboard | `plan/orchestrator/orch-02` | ✅ **covered** (copy-link partial) — `specs/coordination/coordination-panels.spec.ts` drives the three window-dependent assertions against a bound lead writing the shared documents over the **real MCP/IPC wire** (the lead fixture's second arm, selected by a dropped plan file). A stale scratchpad save is refused by the **real** optimistic-concurrency guard: the conflict banner names the revision only the concurrent lead bumped, and reloading restores the lead's content with the window's rejected edit gone. A blocked todo's Complete is refused by the **real** gate (`TodoBlocked` surfaced) until its blocker completes, whose live `TodoChanged` clears the gate and lets it complete. A comment renders the author the core stamped from the lead's **bound session** ("Codex", never "unattributed"). **Copy-link is partial:** reading the system clipboard under WebKitGTK/WebDriver needs a test-only hack (no first-class WebdriverIO clipboard API; clipboard-read is denied under automation — webdriver.io docs verified 2026-07-18), so the `solo://` URL construction stays headless — the core `link` round-trip + scope-resolver tests (`crates/core/src/coordination/link_tests.rs`, `facade/link_tests.rs`) plus new Vitest for the UI `copyLink → writeText` wiring — leaving only the OS-clipboard hop honestly not e2e-proven. Mutation-verified: dropping the SQLite revision guard, the todo blocker gate, and the comment-author stamp each reddens exactly its one assertion, all six other spec files holding. **Reconciled 2026-09-06** when the scratchpad surface became the to-do board's twin: the roster and the editor beside it are gone, so the screen is `ScratchpadBoard` and the walk opens a document by its card, into a detail panel. That moved one *step* of the walk rather than any assertion — the panel lands in **reading**, so the editor is now started before the lead's concurrent write instead of after, because a document merely being read follows the revision the board sees (the pane re-reads it under the reader) and only an open editor holds the revision the next save is guarded by. Reading and writing mount the same rich-text editor, so `startEdit` waits on the Done control, which exists only while editing; waiting on the body would return before anything was editable |
| Todo workspace and agent session context | `coordination` | todo-workspace UX (tracker #74–#78) | A board row shows its `#id`, declared status and the singular/plural unmet-blocker count of a chain the lead built over the wire; the row the lead locked names the lead and opens its terminal; the sidebar's **Todos** and **Scratchpads** groups under the project list the documents the lead holds or touched, each at the role the core derived (**Implementing** for its lock, **Editing** for a tool write, **Reading** for a tool read); a card hands the pane to that todo's detail panel and Back retraces the way in; each sidebar row returns to the surface holding that document, focused; the board's own surfaces have no horizontal overflow at the app's minimum window width; a todo marked done leaves the Todos group while the rest stay; stopping the lead empties both groups | this charter | ✅ **covered** (2026-09-02; reconciled the same day onto the master–detail board; one point pulled as harness-blocked) — `specs/coordination/todo-workspace.spec.ts` drives the lead fixture's coordination arm, extended to `todo_lock` / `todo_get` / `scratchpad_read` over the **real MCP/IPC wire**, so "Current work" and "This session" are what the core recorded, never a repaint. Asserts the id, declared status and `1 unmet blocker` / `2 unmet blockers` of two wire-built chains; the agent control naming the lead with the lead's real process id (the one the lineage tree carries) and opening its terminal; the header's current/session lists; both inbound navigations (each opening that item's detail panel with focus parked on Back); a card opening its detail panel and Back retracing it — the panel really dropped and focus really returned to the originating row; the list panel's toolbar and rows container **and the detail panel's header and scroller** fitting at the `tauri.conf.json` minimum (read, not restated); and the per-run clear on stop, asserted on the stopped lead's re-opened pane — stopping the selected process navigates away, and a read of no pane passed vacuously until the read told the two apart (mutation-verified, e2e-01). **Product defects it caught, all fixed before it went green:** `SessionWorkBar` groups overlapped in a tight header (inline item not hit-testable); the inbound focus effects in `TodoBoard`/`ScratchpadPanel` ran before a freshly mounted pane had its first snapshot; todo-row spans overlapped the agent control at 720px. **Pre-existing, out of scope:** at 720px with the git rail open the orchestration pane is 184px wide and its six-segment view switch needs 485px (`scrollWidth 485 / clientWidth 184`), so the assertion is scoped to the board surface. **Pulled as harness-blocked:** keyboard traversal — the embedded driver synthesizes key actions as untrusted `KeyboardEvent`s, so a Tab cannot move focus and `:focus-visible` cannot be produced (e2e-01); the two stops being separate, non-nested focusables stays with `TodoItem.test.tsx`. New handles: `todoBoard.waitForLock/openAgent/horizontalOverflow`, `sidebar.documentRows/waitForDocumentRole/waitForDocumentGone/waitForNoDocuments/openDocument` (the terminal-header readers `waitForCurrentTodo`/`waitForSessionWorkCleared`/`sessionWork()` went with the header), `harness/window.ts`. **Reconciled 2026-09-02** when the board became master–detail: row expansion is gone, so `expand`/`isExpanded` and the three accessors that read inside the `<li>` were reworked onto the detail panel, and `orchestrationPane.focusedTodoId/waitForFocusedTodo` moved to `todoBoard` (a todo row's focus is the board's fact, and `[data-todo-id]` may not appear in two screens). `todoBoard.view()` is now one atomic snapshot of route + open todo + focus, because a route change swaps both panels' `inert` and moves focus in a single commit. **The reconciliation caught its own defect:** `TodoPanels`' `transitionend` guard tested `propertyName === "transform"`, but Tailwind v4's `-translate-x-full` animates `translate`, so `onSettled` never fired and the retained detail panel was never unmounted — invisible to jsdom, which fires no transitions at all. Fixed in the component (`SETTLE_PROPERTIES`), and carried into `common/SlidingPanels.tsx` when the board was rewired onto it — that primitive settles on `["transform", "translate"]` and its own test asserts the `translate` the browser really emits. **Reconciled again 2026-09-06** when the scratchpad surface became the same master–detail board. Activating a scratchpad from the terminal's session bar now opens that document's **detail panel with focus parked on Back** — the shape the to-do half of this walk already asserted — so the roster-row focus read went with the roster; `scratchpadPanel` is `scratchpadBoard`, taking route, open document and focus in the one atomic snapshot `todoBoard.view()` takes, for the same reason. The to-do half needed only handle renames, both boards now wearing one kit: `data-todo-route`/`-back`/`-trigger`/`-toolbar` are `data-panel-route`, `data-detail-back`, `data-card-trigger`, `data-board-toolbar`. **And the reconciliation caught a product regression neither board's own tests could see.** `OrchestrationPane` seeded its view from `useState<View>("agents")` and switched on `focus` *changing* — but the inbound navigation deselects the process and names its target in one commit, so the pane it lands in is always a fresh mount and there is no later render for that switch to happen in: the reader arrived on the agents tree instead of the item they activated, from either half of the session bar. Introduced when the switch moved from an effect to a render-time adjustment (`ae4c570`) — the effect ran on mount, the adjustment cannot. Fixed by seeding the view from the activation, with the mount case now covered in `OrchestrationPane.test.tsx`, which had no `focus` test at all. **Reconciled 2026-09-08** when the session context moved out of the terminal header into the sidebar: a project's live work is now two groups under the project node — Todos and Scratchpads — each row a plain button (never a treeitem, so the arrow-key walk keeps addressing process rows) carrying the document's handle and the strongest role of its live participants. The "Current work" / "This session" reads are gone with the header; the walk reads the groups atomically off `data-document-section`/`-row`/`-todo`/`-scratchpad`/`-role` and opens a document by clicking its row (the embedded driver cannot Tab to it). **The reconciliation corrected its own expectation:** the plan called the todo the lead read through a tool "read-only", but the lead *created* it through a tool too, which the core records as a write, and a later read never downgrades a recorded write — so the fixture renders it **Editing**, and the walk asserts exactly that. The **Reading** role is therefore not exercised by this fixture (it would need a todo the lead did not create) and stays with the core's `project_work` unit tests. Two steps changed shape: the stop step no longer re-opens the stopped lead's pane, since the sidebar is always on screen, and it is the acceptance test for the session record announcing its own drop — the close hook releases the todo lock straight into the store without publishing, so nothing else tells the window the rows are gone. A new step marks the lead's locked todo done through the board's detail panel (the stub has no complete verb; its blocker first, since the gate refuses otherwise) and watches each leave the group on its own while the read todo and the scratchpad stay — the group's Done filter, asserted whole. **Static only as of this note:** the walk was reworked without a built app and has not yet run; the first live run owes the roles actually rendered, whether the two Done drops re-read fast enough on `TodoChanged`, and whether `openDocument` needs the `select()`-style click retry |
| Filling in a prompt template | `coordination` | F15 (prompt-template render) | The placeholders a prompt declares become value fields; answering some and not others shows the prompt an agent would receive, with every gap still visible and named | this charter | ✅ **covered** — `specs/coordination/prompt-templates.spec.ts` opens the Templates manager on a project's own library and drives its preview against the **real renderer**. The load-bearing assertion is the unanswered placeholder: with one of two values typed, the prompt keeps the literal `{{focus}}` *and* an advisory names it. Silent substitution of an unanswered fill-in was refused in the design and is enforced in the core — but only by treating an absent value as absent, and the window is what decides absence: an emptied field defaulted into the values map would answer every placeholder with nothing. The headless tests of this (`TemplatePreview.test.tsx`) run against a hand-written stand-in for the core's render, so they prove the window's half against our *belief* about the core's; this is the only thing that puts the real renderer and the real window on one screen. Also walks the emptied-field case (the marker returns) and the cleared-description round trip (the UI's `Some("")`-clears / `None`-keeps contract against the real store). Scope groups, per-scope empty states, and refused delete/duplicate stay headless — `TemplatesPanel.test.tsx` answers them in jsdom. Mutation-verified |
...
| In-window alerts & unread | `monitoring` | D8–D10 (the half inside the webview) | An alert about a process the user is *not* watching reaches the window as a toast, marks that row and its project, and counts in the title bar; acting on the toast selects the process; clearing empties every indicator at once; the process the user *is* watching raises nothing at all; a project set to None drops a crash outright; a command's own level — and inheriting, which is stored as the absence of one — survive the settings pane closing and reopening | this charter | ✅ **covered** — `specs/notifications/` (three files, 11 assertions) drives the real supervisor and the real terminal parser: the crash is a stub exiting nonzero on a cue file and the script alert is a real `OSC 777` sequence the stub printed, so no repaint can fake either. Two cue-driven stubs join the `basic` fixture (`cued-crasher.sh`, `signaller.sh`) — cued rather than immediate because starting a process from its row selects it, and the core suppresses an alert about the process being watched, so a signal that must reach the user has to land after the spec has looked elsewhere. New screens `ToastStack`, `AttentionControl`, `ProjectSettingsPane`. **Window focus is asserted, never arranged** (`harness/windowFocus.ts`): the core routes an alert to the desktop rather than a toast for a user who is not looking, so "no toast" is also what an unfocused window produces — without the tripwire the suppression walk would be unfalsifiable. Mutation-verified eight ways (e2e-01) |
| Desktop notification | `monitoring` | Phase 6 (D8) | A crash raises a libnotify desktop toast; clicking it focuses the terminal (needs a real notification daemon) | `phase-06` | ⬜ not built — the **native** path only; everything the same alert does inside the webview is the row above. Not reachable from WebdriverIO at all: the desktop notification, its sound, and the app-icon badge are window-system and audio surfaces outside the page |
| Reading a change | `version-control` | VC2 (diff viewer) | Opening a changed path from the version-control rail shows both sides of the change, and leaves the window standing | this charter | ✅ **covered** — `specs/version-control/open-diff.spec.ts` opens a project the harness has made a real repository holding one uncommitted change, clicks that path in the rail, and reads the split: the working tree's line **and** the line only the commit still holds, so the diff can only have come from the core really comparing the two. The load-bearing half is the second assertion. The diff surface is the app's one on-demand chunk with a heavy third-party viewer in it, and a module that throws while it is being brought in takes React's single root with it — the window empties rather than merely missing a diff. Nothing headless reaches this: the bundler decides what the viewer's dependencies resolve to (the app aliases `lowlight` away to keep its grammars out of the chunk) and Vitest resolves them for itself, so the two never meet outside a real build. New: a `makeRepository` harness step, an `openProject` prepare hook, `GitRail` + `DiffPane` screens. Mutation-verified: dropping one method from the `lowlight` stand-in reddens **only** this spec file, with the captured page source showing an empty root |
...
| A working agent's name | `agents` | — | The highlight on a working agent's name is sized to the word, not the row, so the band sweeps *through* the name instead of lighting it all at once | this charter | ✅ **covered** — `specs/agents/working-shimmer.spec.ts` measures the highlight's box, the name's own glyph run and the row's label cell in one layout pass. The glyph run is read with a range over the text node rather than any box enclosing it: the highlight fills that box, so comparing the two could only ever agree with itself. Only a real window answers it — jsdom lays nothing out, so every box measures zero and `ProcessRow.test.tsx` stayed green throughout the defect, asserting the overlay merely exists. Mutation-verified against the pre-fix geometry: overrun 74px against a 1px tolerance, failing on geometry rather than on a timeout or a selector miss. A hidden highlight measures zero exactly like a collapsed one, so the read names reduced motion instead of reporting a bare zero |

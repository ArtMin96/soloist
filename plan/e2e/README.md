...
| Addressed agent messaging | `orchestration` | orch-04 (O13/O15/O16) | A default `spawn_agent` receives onboarding after idle while an explicit opt-out does not; prompt Tasks, no-todo direct/group messages, list/get/ack, and the optional durable todo completion cross real PTYs and authenticated IPC | `plan/orchestrator/orch-04` | ⏳ **source added — pending run** — `specs/orchestration/agent-messaging.spec.ts` uses two long-lived fixture workers. The primary's request serializes the default-true setting without an `include_agent_instructions` field and prints proof only after the briefing arrives as a submitted PTY turn; the peer explicitly sends `false` and fails if the briefing appears. Their direct/group exchange deliberately carries no todo correlation; only the final durable board/result path uses `agent_report_completion`. The fixture stays alive after each proof so a fast scenario cannot race the sidebar's `Running` observation |

...
| O9 | `spawn_process` (arbitrary terminal over MCP) with its trust treatment — future work, including its O13 onboarding leg | ✅ name / ❓ trust | orch-04 | v1 | Trusted spawn works; untrusted / cross-project refused; O13 behavior matches `spawn_agent` |
...
| O13 | **Spawn orchestration onboarding + optional first task** — `spawn_agent` instructions default ON; an optional prompt queues an addressed `Task` with optional todo correlation; Soloist waits for the worker's first `Idle` transition before submitting a compact wake turn. The `spawn_process` leg remains partial until O9 lands | 🟡 briefing / ❓ task transport | orch-04 | v1 | Default `spawn_agent` onboarding arrives after idle; opt-out suppresses it; a prompt is retrievable/acknowledgeable without a todo and never enters CLI arguments or startup input; repeat for future `spawn_process` |

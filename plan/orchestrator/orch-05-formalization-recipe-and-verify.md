...
`AGENTS.md`/`CLAUDE.md`); a full-loop UI acceptance walk (Playwright); the `O1–O16` parity walk + ledger
...
1. **Orchestrator recipe (O11, [`06` §5](../06-codebase-blueprint-and-cleanup.md) style):** document the
   ...
   (O14) → optionally derive a blockered todo chain when durable board/result tracking is useful →
   promote a lead → `spawn_agent` workers with an optional first `Task` and default-on reusable
...
2. **Agent-facing guidance (O11):** write the orchestration section for `AGENTS.md`/`CLAUDE.md`
   (primitives, identity/binding, roster + mailbox retrieval/acknowledgement, completion reporting,
   fire-when-idle, and the "don't busy-poll, set a timer and sleep" rule) so a
   freshly bound agent can discover and use the loop with **no skills loaded** — matching the demo. The
   **runtime O13 briefing (orch-04) is the primary discovery path** (it reaches a spawned worker after
   the first idle transition); this static `AGENTS.md` content is its **complement** for human-launched / externally
...
3. **Full-loop UI acceptance walk (Playwright):** drive the whole loop and assert it is **visible** end to
   ...
   backstop.
4. **Parity walk + evidence (O1–O16):** run each `O`-row's Verify, record pass/fail with evidence (test
...
- Every `v1` `O`-row (O1–O16) passes its Verify with recorded evidence; all CI gates green; the

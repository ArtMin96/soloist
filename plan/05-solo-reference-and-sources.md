...
| MCP session↔process binding authenticity | the bound `ProcessId` comes from the `SOLOIST_PROCESS_ID` Soloist injects, and `bind_session_process` validates only that the process exists. While the MCP tools are read-only (open by design, F13) this crosses no project boundary. Verifying the caller actually runs in that process (peer-credential → process-group match), on which cross-project scope isolation rests, lands with the action-tool trust + scope gate (F13), not the skeleton |

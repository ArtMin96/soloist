...
| Auto-summarization headless invocation (E6) | Solo documents Claude/Codex/Gemini native-headless summarization (default models `sonnet`/`gpt-5-codex`/`flash-lite`, 15s/30s/1min cadence); the per-provider invocation and prompt are ours (clean-room). Each supported provider composes its own documented headless one-shot — Claude `-p`, Codex `exec`, Gemini `-p`, OpenCode `run` (each `--model`/`-m` when a model is set), and a user-configured Generic tool via its `PromptMode` (appended arg or piped stdin) — grounded per arm in that provider's published CLI reference. Amp/Copilot/Kimi document no id-less headless one-shot we can ground, so they yield **no summary** rather than a guessed flag (the resume-Strategy's honesty). A compact rendered-text snapshot (last ~40 lines, ≤4 KB) runs through the login shell (`$SHELL -lc`, in its own process group, killed+reaped on a 45 s bound); summarization is **OFF by default** (opt-in tool+model in settings), per-agent rate-limited (30 s cooldown, within Solo's cadence band), and fully degradable (no runner / unsupported / failure → no summary, never blocks the core). Ours (clean-room) |

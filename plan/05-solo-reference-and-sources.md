...
| MCP session↔process binding authenticity | the bound `ProcessId` comes from the `SOLOIST_PROCESS_ID` Soloist injects, but `bind_session_process` validates only that the process exists and `select_project` only that the project is loaded — neither verifies the caller actually runs there. The scoped **action** tools (process control, bulk start/stop/restart, `clear_output`, `spawn_agent`) enforce effective-project scope on top of this, so with **≥2 projects open** a client on the (same-user, `0700`) socket could select a sibling project's scope and mutate it — and `stop_all_commands`/`restart_all_commands`/`clear_output` are not trust-gated. With 0 or 1 project open the scope is unambiguous and there is no cross-project surface. Verifying the caller actually runs in that process (peer-credential → process-group match), on which cross-project isolation rests, is the deferred **F13** authenticity check — it lands once over all the action tools, not per session. Tracked in `KNOWN-DIVERGENCES.md` D-6 |

...
> **E6 (Optional auto-summarization) — `Done — pending verify` (2026-07-01, 3 stacked branches off `main`):** Realized the opt-in, OFF-by-default agent auto-summarization end to end. The settings opt-in (`summarizer_tool`/`summarizer_model`) already existed; this session added the behavior. **Architecture (reuses existing patterns):** a per-provider **Strategy** `summary_strategy_for(kind)` in `core::agents::summarize::strategy` composes each provider's *documented* headless one-shot — Claude `-p`, Codex `exec`, Gemini `-p`, OpenCode `run` (model via `--model`/`-m`), Generic via its `PromptMode` — each arm cited against the provider's CLI reference (verified via WebFetch this session); Amp/Copilot/Kimi have no grounded headless one-shot → **no summary** (honest, like `NoResume`). A generic **`SummaryRunner` port** (Noop default) runs a composed invocation off-runtime; a bus-driven **`SummaryReactor`** (mirrors `NotificationReactor`) reacts to the `Idle` transition, reads the live opt-in, runs the summary cadence-limited (30 s/agent), and publishes a new **`DomainEvent::AgentSummary`** — fully degradable (no runner/unsupported/failure → nothing, never blocks core). Real adapter **`sys::CommandSummaryRunner`** runs `$SHELL -lc` in its own process group (killed+reaped on a 45 s bound; `wait_with_output` on a thread drains stdout). UI: a muted 11px caption under the agent's sidebar row, cleared on leave-Running (designed via `/impeccable`; `summary-in` motion-safe enter). Removed the empty `Summarizer` marker port and the orphaned `core::idle` stub; extracted `sys::shell::login_shell` (DRY) and `tool::shell_command_line` (DRY). **Branches (stacked, in merge order):** `feat/e6-agent-summarization-core` (tip `95ea524`) → `feat/e6-agent-summarization-adapter` (tip `f9f7940`) → `feat/e6-agent-summarization-ui` (tip has the UI + this doc commit). Each branch carries its feature commit plus a follow-up **session-isolation fix** (below). **Session-isolation fix (owner-reported bug, 2026-07-01):** the summary described the *dev session / project* instead of the target agent's session, because the summarizer subprocess inherited the app's cwd and an agent CLI used as summarizer (e.g. `claude`) loaded that dir's ambient context (CLAUDE.md, per-project memory, prior sessions). Two-part fix: `fix(core)` binds the prompt to the fenced `<transcript>` only (forbid other files/tools/memory/self); `fix(sys)` runs each invocation in a **fresh, empty, per-run temp dir** (removed after) so no project context can leak. Snapshot routing was already correct per-agent (`rendered_tail(id)` + `AgentSummary{id}`). **Evidence:** core **508 tests** (21 new summarize: strategy/prompt/reactor decision+publish+degrade+cadence on mock clock), sys adapter tests (stdout/stdin/nonzero-exit/timeout-kill via `/bin/sh`), UI **216 tests** (signal fold/clear + caption render/absent); `clippy -D warnings`, `rustfmt`, `tsc`, ESLint, Prettier, dep-direction all green; UI production build compiles. Clean-room decisions in `KNOWN-DIVERGENCES.md` **D-13** + `plan/05` §12; matrix row E6 updated. **Not run:** owner runtime walk (enable a summarizer tool, watch a real agent idle → caption appears) — the "summary when enabled" Verify half; "disabled OK" is structural. **Next session should start with:** open the 3 stacked PRs (via `gh`, ArtMin96 token, in order, no self-merge), run `just lint && just test` once on the top branch to reconfirm the whole gate, then the owner runtime walk. E6 was `later`; built ahead per owner request — it does **not** change any v1 phase status.
>

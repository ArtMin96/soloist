...
- **R8 review + served-surface guard (2026-06-23, PR #21 open):** an independent review confirmed the split is
  ...
  Phase 9's ~50 coordination tools. **Pushed; PR #21 open.**
...
**A. Phase 8 is `Done — pending verify` (all v1 F-rows code-complete, all PRs merged to `main`) — merge PR #21
+ run the user-only runtime acceptance walk to flip Phase 8 `Verified`, then start Phase 9.**
...
session on branch **`refactor/mcp-tool-router-split`** (off `84230ca`, commits `150cd88` refactor + docs) —
**pushed; PR #21 open**, reviewed, with a review-requested served-surface guard added to `server_tests.rs`
(`tool_router.list_all()` == the 28 expected tools; mcp 31 → 32). **Next, in order:**
1. **Review + merge PR #21 (`refactor/mcp-tool-router-split`) to `main`.** Pure code movement (server.rs 546 → 46;
   seven `tools/<cat>.rs` sub-routers composed via `ToolRouter` `Add`); tool surface byte-identical and now
   asserted by the served-surface test; gate green. See the top Decisions entry.

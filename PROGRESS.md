...
- **Overall:** **Phase 7 (Agents & idle detection) — `Verified` (all v1 rows E1–E5, E8; user-confirmed at
  ...
  (rmcp stdio → IPC/UDS → app → `Facade`; identity/scope + 5 read tools; F1/F3/F4 + read F5/F6) — landed on
  `feat/phase-8-mcp-skeleton`, gate green; see the top Decisions entry + "Next session" item A for the tool
  fan-out.** _Prior detail:_ **Phase 7 (Agents & idle detection) — all v1 rows (E1–E5, E8) code-complete; `Done — pending

...
- **Phase 9 — timers (G7–G9) landed (2026-06-24), the headline token-free fire-when-idle.** Second C6 aggregate,
  built end-to-end on the proven ports/adapters structure (branch `feat/phase-9-timers` off `main` `9600b6e`; **PR
  not yet opened** — merge is the user's call). Same vertical the leases slice set: behavior → C6 context →
...
**A. Phase 9 is IN PROGRESS — lease locks (G6) AND timers (G7–G9) landed. Next: scratchpads G1/G2.**
PR #22 (the leases slice) is **MERGED to `main`** (`9600b6e`). Timers landed this session on branch
`feat/phase-9-timers` (off `main`) — gate green (core 288 / store 38 / ipc 14 / app 30 / mcp 44 / sys 5 +10 /
pty 1 +12 +3 ign / UI 78; `just lint` + `just test` + `cargo check -p soloist-app --no-default-features` all pass).
**Merge of `feat/phase-9-timers` is the user's call — do NOT self-merge; ask.** **In order:**
1. **Review + merge the timers slice** (`feat/phase-9-timers`). Additive vertical on the established
   ports/adapters structure (C6 `Timers` aggregate + `TimerRepo`/`NoopTimerRepo` + a self-supervised
   `TimerScheduler`; SQLite `TimerRepo` + migration **v5**; 7 MCP tools; ipc arms; scheduler spawned + timers
   reconciled-on-launch in the composition root). Clean-room timer semantics recorded in `plan/05` §12.
2. **Next slice — scratchpads G1/G2** (revision-guarded writes — the **optimistic-concurrency** pattern, still
   "to add" per `plan/06` §4 / ARCHITECTURE §3): a `ScratchpadRepo` over the store (migration **v6**), `_write`
   takes an `expected_revision` and returns `RevisionConflict` on a stale write; leading H1 = title; read modes
   full/headings/section; the `scratchpad_*` tool family (`05` §7). Then **todos G3–G5** (tags + blockers +
   comments + transfer + **process-owned todo locks** that reuse the existing `LockReleaser` close hook), **kv
   G10** (default off), then **E7** end-to-end (`spawn_agent` → assign todo → lock → `fire_when_idle_all` →
   integrate on wake). Each a thin handler → one `Facade` method → existing core behaviour; follow the SAME
   vertical the leases + timers slices set; consult the **MCP spec** (`modelcontextprotocol.io`, current rev
   **2025-11-25**) + `rmcp` via context7 (mcp-builder is not installed).

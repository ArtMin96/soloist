...
> **ACTIVE PHASE: 11 (UX Polish & Execution Profiles) — STARTED. First slice landed: I10 env capture (`$SHELL -ilc env`, ~10-min cache, precedence process > captured > app). Phase 10 (HTTP API & CLI) is `Verified` — the user-only runtime acceptance walk PASSED 2026-06-25 (status/restart/logs against a live app, CLI/UI restart route identically, app-down → "Soloist is not running"/exit 1, mutation auth 200-vs-401, foreign-Origin CORS refused). Phase 9 (Coordination, C6) is `Verified`.**
...
- **Phase 11 slice 2 started on `feat/phase-11-settings-mcp-toggle` (off `main` `17f0115`).** The backend
  foundation the UI rows need: `SettingsRepo` (migration v9) + the per-group MCP tool-enablement Registry
  behind the one `Facade`, carrying in the G10 Key-Value default-OFF gate (plan/05 §7).

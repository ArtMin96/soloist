...
**A. Phase 8 is `Done — pending verify` (all v1 F-rows code-complete) — open/merge session 5's PR, run the
runtime acceptance walk to flip it `Verified`, then start Phase 9.**
Sessions 1–4 are merged to `main`. **Session 5 (this session)** landed the **F6 remainder** on branch
`feat/phase-8-mcp-f6-remainder` (`950e559`, off `main` `633c862` = PR #19 merged) — `rename_process`
(new C2 `Supervisor::rename` + `DomainEvent::ProcessRenamed`, mirrored FE), `close_process` (async C2
`Supervisor::close`: reuse the stop path, **await the reap**, then `remove`; emits the existing
`ProcessRemoved`), `select_process` (informational C8 `selected_process`, reported by `whoami`; no scope
conferred — YAGNI). Each a thin `mcp` handler → one `Facade` method → an `IpcRequest` arm; scope/trust in the
core; clean-room per-tool semantics in `plan/05` §12; tests in their own files. Gate green: core 252 / ipc 13 /
app 28 / mcp 31 / store 15 / sys 15 / pty 13 (+3 soak ign) / UI 78; clippy `-D`, fmt, tsc, eslint, dep-guard,
soak (3), `--no-default-features` all green. **The full v1 F-set — F1, F3–F11, F13 — is now code-complete.**
**Next, in order:**
1. **Push the branch + open its PR (ArtMin96), review, merge to `main`.** Not yet pushed/PR-opened.
2. **Runtime acceptance walk (user-only — desktop `just dev` + an MCP client), then flip Phase 8 → `Verified`.**
   Per `plan/phases/phase-08` Acceptance: launch the app + a fixture stack, drive `soloist-mcp` via the MCP
   Inspector / a scripted stdio client, and observe with evidence — `list_processes` returns the live stack;
   `get_process_output`/`_raw` rendered/raw; `get_process_ports` lists a dev server's port; `whoami` resolves
   the bound process; `send_input` + `wait_ms` returns the tail; `restart_process` recovers a crashed service;
   **`rename_process` relabels a row, `close_process` stops + removes it, `select_process` is echoed by
   `whoami`**; an **untrusted** action is refused and a **forged bind/select to a sibling project** is refused
   (F13); app **not** running → a clear non-crashing error.
3. **Then Phase 9 (Coordination layer, v1)** — scratchpads/todos/timers/leases/kv; the metaharness centerpiece,
   high-risk; unblocks E7. Sequence per the Phase-9 file (durable store → leases → timers → scratchpads/todos →
   kv); invoke `mcp-builder` if it gets installed, else the official MCP docs + `rmcp` via context7.
**Deferred (tracked, non-gating):**
  - **R8 — split the MCP tool router by category** (`plan/06` §7, user-requested 2026-06-23). `mcp/src/server.rs`
    is now **546** non-test lines (the file-size guard's sole outlier), one flat `#[tool_router]`. Split into one
    `mcp/src/tools/<category>.rs` sub-router per logical group (identity/project/process/bulk/output/services/
    agent) with named `pub(crate)` routers (`#[tool_router(router = …, vis = "pub(crate)")]`, the verified rmcp
    1.7 attribute), composed in `SoloistMcp::new` via rmcp's `ToolRouter` `std::ops::Add`; shared reply helpers
    (`structured`/`acked`/`app_error`/`unexpected`/`port_wait_json`) to one `tools/reply.rs`. A Registry/
    composition pattern so a new tool lands in its category file and reuses helpers — easy to extend/maintain.
    Pure code movement; tool surface stays byte-identical (assert via `server_tests.rs`).

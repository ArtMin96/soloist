...
- **Overall:** **Phase 5 (Dashboard UI) — interactive core slice — `Done — pending verify`.** The visible
  ...
  UI +4: grouping ×3, projection ×1). **Pending verify (session constraints, not failures):** on-screen
  rendering, live terminal I/O, and the Playwright e2e are **not yet observed** — GUI auto-launch was
  denied and Playwright/`tauri-driver` aren't installable offline. The gate covers the logic; the rest is
  **manual** (`just dev`, the host has `DISPLAY=:0`).
...
2. **VERIFY THE SLICE FIRST (the gap this session couldn't close).** The logic is gate-green but the
   running UI was never observed. Do one of:
   - **Manual (fastest; host has `DISPLAY=:0`):** `just dev`, then confirm: the three groups render
     (Agents `assistant`, Terminals `shell`, Commands `web`/`build`) all `Stopped`; **Start all** turns
     the Command dots green and streams; selecting a process opens its **xterm** terminal; typing reaches
     it (`echo hi` in `shell`); per-row stop/restart reflect in ~1s; dot colors + glyphs match DESIGN.md.
     Watch for the one untested boundary — the `Channel<Vec<u8>>`→`Uint8Array` delivery + rAF coalescing
     in `useTerminal.ts` (if bytes arrive as `number[]`/`ArrayBuffer` instead of `Uint8Array`, coerce
     before `term.write`). Also confirm **contrast** of the status hues in both themes (impeccable AA).
   - **Playwright e2e (Task 6, blocked offline this session):** install `@playwright/test` + browsers,
     then assert grouping, `[data-status]`, control enable/disable, selection, empty state against the
     real app via **`@tauri-apps/api/mocks` `mockIPC`** (already installed) injected with a fixture stack;
     full PTY echo needs `tauri-driver` + `WebKitWebDriver`. Drive via the `webapp-testing` skill.
3. **Then build the deferred Phase-5 follow-up** (still `/impeccable`-driven): **trust dialog** (A6/A9 —
...
4. **Wire "open project → register config commands → `reconcile_orphans()` → `start_all`"** (connect
...
5. **Smaller follow-ups recorded below:** generate the `.impeccable/design.json` sidecar once components
...
6. **Still open from Phase 1 (independent, user-only):** confirm the in-GUI Start/Stop click via `just
   dev`, then flip the Phase 1 row to `Verified`.
7. **Do not pull deferred `later` rows into v1** (A5/A8/A10/A12/A13, B9, C8 webgl) and **do not** build

...
**A. Phase 9 is IN PROGRESS — lease locks (G6) landed on branch `feat/phase-9-leases` (PR open). Merge it, then
build the next aggregate (timers G7–G9).**
The leases slice is gate-green and **review-fixed** (core 268 / store 25 / ipc 14 / app 30 / mcp 37 / sys 5 +10 /
pty 1 +12 +3 ign / UI 78; `just lint` + `just test` + `cargo check -p soloist-app --no-default-features` all pass).
The review's fix-then-ship findings are applied: the `LockRepo` acquire is now atomic (concurrent-acquire race
closed, 16-thread test) and the lease TTL default/bounds live in the core. **In order:**
1. **Review + merge the leases PR** (`feat/phase-9-leases`). Additive vertical slice on the established
   ports/adapters structure; gate green; review findings applied.
2. **Next slice — Timers (G7–G9), the headline token-free fire-when-idle.** Sequence per the Phase-9 file. Build on
   the substrate: `TimerRepo` over the store (migration v5), a `Clock`-driven scheduler, `timer_set` delivers `body`
   to the owning agent as a fresh turn (via the bound process's `send_input`), `timer_fire_when_idle_any/all`
   subscribe to C4 `AgentActivityChanged`, `timer_cancel/pause/resume/list`. **Reuse `Clock::now_unix_millis`** for
   `At(deadline)` (the wall clock added this slice). New `tools/timer.rs` sub-router → `SoloistMcp::new`'s `Add`;
   extend `EXPECTED_TOOL_SURFACE`. Then **scratchpads G1/G2** (revision-guarded — the optimistic-concurrency
   pattern, still "to add"), **todos G3–G5** (blockers + process-owned locks reuse the `LockReleaser` hook), **kv
   G10** (default off), then **E7** end-to-end (`spawn_agent` → assign todo → lock → wait-idle → integrate). Each a
   thin handler → one `Facade` method → existing core behaviour; consult the **MCP spec** (`modelcontextprotocol.io`,
   current rev **2025-11-25**) + `rmcp` via context7 (mcp-builder is not installed).

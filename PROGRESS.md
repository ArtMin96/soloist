...
- **Newest (2026-06-23, Phase 8 session 4):** **F13 binding/scope authenticity landed**, closing
...
**A. (Phase 8 session 5) Finish the F6 remainder, then close out Phase 8.**
Sessions 1–4 are done. Session 1 (skeleton: F1/F3/F4 + read F5/F6), Session 2 (F6 process control + F7 + F11 +
F13 scope), and Session 3 (F8 bulk + F9 output + F10 services, PR #18) are all **merged to `main`** (HEAD
`b8d51c0`). **Session 4 landed F13 binding/scope authenticity on branch `feat/phase-8-mcp-binding-auth`
(`b8b2cb5` + the `facade/session.rs` extraction `20e4e71`) — gate green; review/merge it (open a PR) first.**
That **closed `KNOWN-DIVERGENCES.md` D-6 (→🟢)**: scope is now authenticated by the peer's `SO_PEERCRED` →
process-group match, so a forged bind/select to a sibling project is refused — see the top Decisions entry. The
full path **agent → `soloist-mcp` (rmcp stdio) → IPC/UDS → app → `Facade` → core** is proven for read AND action
tools. **Done across S1–S4:** all of F1/F3/F4/F5, F6 control/F7/F8/F9/F10/F11, and **F13 (scope + trust +
binding authenticity)**. **Still to add — the F6 remainder** (each a thin `crates/mcp` handler
[clean-room `schemars` params] → one new/existing `Facade` method → an `IpcRequest` arm in the **async**
`handle_request`; no domain logic in the handler; scope/trust in the core):
  - **`rename_process`** — new C2 `Supervisor::rename(id, new_label)` updating the registry's `label` +
    emitting a new `DomainEvent::ProcessRenamed { id, label }` (mirror in `domain.ts` + the projection). Scoped
    façade wrapper in `core/facade/scoped.rs` (uses `require_in_scope`).
  - **`close_process`** — **new C2 behaviour**: stop the group (reuse the stop path) + **remove from the
    registry** (there is no removal path today — the registry only forgets pgids). Emit the existing
    `DomainEvent::ProcessRemoved { id }` (defined + consumed by `restart::clear_restart_tracking`, but **never
    emitted in production yet** — wire it here). Mirror the removal in `domain.ts`/projection. Scoped wrapper.
  - **`select_process`** — a session **selected-process** field in **C8 `identity`** (mirrors `select_project`);
    `whoami` reports it. Decide its only consumer (likely just `whoami` reporting / a default target hint —
    our tools all take explicit ids, so keep it informational unless a real consumer appears — YAGNI).
Then **close Phase 8**: confirm every v1 F-row (F1, F3–F11, F13) passes its parity Verify with evidence, run
the acceptance walk (MCP Inspector / scripted client against a live app + fixture stack, `plan/phases/phase-08`
Acceptance), and flip Phase 8 → `Done — pending verify` (then `Verified` after the runtime walk). **F13 is
done** (scope + trust + peer-credential binding authenticity, session 4) — its Verify now includes "a forged
bind/select to a sibling project is refused."
**Deferred beyond Phase-8-core:** **`spawn_process`** (arbitrary *terminal* command over MCP — trust-sensitive,
outside `launch_agent`; design its trust treatment before adding) and the **F1 helper bundling**
(`externalBin`/sidecar — a **Phase-12** packaging item; don't pull forward). **F2/F12/F14 stay `later`.** Keep
slices small + gate-green; each its own commit. Use the **official MCP docs** + `rmcp` via context7 (the
`mcp-builder` skill is **not installed**). **Advisory file-size split-candidate:** `mcp/src/server.rs` (494, one
rmcp `#[tool_router]` block) — evaluate rmcp sub-routers to split tools by category if it keeps growing.
Recorded gaps live in `plan/05` §12 (per-tool MCP semantics + the param-schemas-are-ours note).

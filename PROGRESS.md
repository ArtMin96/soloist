...
- **Overall:** **Phase 6 (Monitoring / self-healing) — `In progress`; Phase 5 (Dashboard UI) — `Done —
  ...
  Gate **224 = Rust 182 / UI 42** (+11 Rust filewatch tests; mock-clock, 10× deterministic). Branch
  **`feat/phase-6-file-watch`** (new PR), off `main` (PR #8 merged). The prior **OS-probe slice (D1/D2/D3)**
  merged as **PR #8**; the **crash auto-restart policy** (D4 + D11) as PR #7. **Next: the `notify` OS adapter
  in `crates/sys` (wires D6/D7 live + dynamic re-watch on project open), then D8 notifications.** See the top
  Decisions entry.

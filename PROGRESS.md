...
- **Overall:** **Phase 6 (Monitoring / self-healing) — `In progress`; Phase 5 (Dashboard UI) — `Done —
  ...
  **`feat/phase-6-file-watch`** (new PR), off `main` (PR #8 merged). The prior **OS-probe slice (D1/D2/D3)**
  ...
  `feat/phase-6-file-watch`). Gate **234 (Rust 192 / UI 42)**. **Next session should start with: the nightly
  soak gate** (a longevity CI job, Phase 6 onward — start/stop + crash-restart loop asserting flat PID/FD/task
  counts), then the Phase-6 **UI surfacing** (CPU%/RSS, ports, restart-exhausted/file-restart badges, via
  `/impeccable`). Also open: **R7** (driven-port ownership drift, `plan/06` §7). See the top Decisions entry.

...
- **GIT INTEGRATION — the forge and pull-request create — `Done — pending verify` (2026-08-09, branch `feat/git-forge-pr-create` @ `6b989bf`, stacked on `feat/git-oneshot-env`; todo 43, matrix row VC8).** A branch's commits can be proposed as a pull request from the rail: the branch is published first if the remote does not hold it, the description starts from the repository's own template, an agent can fill that shape in, and what comes back is the address the service printed. **Soloist stores no token and never sees one** — `gh` owns the account entirely, the same bargain `crates/git` makes with the user's own credentials. Nothing here names `github.com`: which service a repository belongs to is the tool's own job, so an enterprise host is answered by the same invocations.
  ...
  - **Next session should start with:** **PR 8 (todo 44) — PR review and the agent handoffs** (comments/reviews, CI check runs with live status, merge, "fix this failing check" / "address this comment"). The seam it attaches to is `PullRequestSummary`, which renders a pull request today and offers nothing but a link. `GitForge` is where `CheckRun`/`ReviewThread` land, and `gh pr view --json statusCheckRollup,reviews,comments` is the read. Owner decisions still owed from earlier slices: `commit.template`, and the diff-pane renderer that PR 3's containment probe left open.
...
**◆ NEW (2026-08-09) — git integration: PRs 0–7 are built; start PR 8, review and the agent handoffs
(todo 44).** Read the Soloist scratchpad **`git-integration-design`** §3 (the review half of the base
list) and §8 item 8, then `plan/02` **VC9** — its Verify column is the definition of done: review
threads and check runs with live in-progress/pass/fail, a merge that performs the requested method and
reports the forge's refusal without merging, and a handoff that delivers the **full** context (check
name + log excerpt, or the comment with its file and line) into the bound agent's session as one fresh
turn.

It attaches to `PullRequestSummary`, which renders a pull request today and offers nothing but a link.
`GitForge` is where `CheckRun` and `ReviewThread` land — deliberately not built in PR 7, since nothing
consumed them — and the reads are `gh pr view --json statusCheckRollup,reviews,comments` and
`gh pr merge`. Non-negotiables it must not break: no prose is classified in `crates/forge` (PR 7 chose
`gh pr list` over `gh pr view` precisely to keep that true), a proposal-shaped write is trust-gated in
the core, and `create`'s rule holds for `merge` too — an accepted action must never be reported as a
failed one.
...
Build order after that: PR 9 history → PR 10 MCP tools (todos 45–46).

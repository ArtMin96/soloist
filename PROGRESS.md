...
**A. (Phase 8 session 4) Finish the F6 remainder, then close out Phase 8.**
Sessions 1–3 are done and on branch PRs. Session 1 (skeleton: F1/F3/F4 + read F5/F6) is **merged to `main`**.
Session 2 (F6 process control + F7 + F11 + F13) is **merged to `main`** (HEAD `69c56da`). **Session 3 (this
session) landed F8 bulk + F9 output + F10 services on branch `feat/phase-8-mcp-bulk-output` — PR open, gate
`just lint` + `just test` exit 0; review/merge it first.** The full path **agent → `soloist-mcp` (rmcp stdio) →
IPC/UDS → app → `Facade` → core** is proven for read AND action tools. **Done across S1–S3:** all of F1/F3/F4/F5,
F6 control/F7/F8/F9/F10/F11/F13. **Still to add — the F6 remainder** (each a thin `crates/mcp` handler
...
Acceptance), and flip Phase 8 → `Done — pending verify` (then `Verified` after the runtime walk).
**Deferred beyond Phase-8-core:** **`spawn_process`** (arbitrary *terminal* command over MCP — trust-sensitive,
outside `launch_agent`; design its trust treatment before adding), **F13 binding-authenticity** (peer-cred →
pgid; tools are currently bound-by-trust-of-`SOLOIST_PROCESS_ID`, `plan/05` §12), and the **F1 helper bundling**

...
0. **Phase-5 follow-up — CODE-COMPLETE (2026-06-19).** All follow-up pieces landed, each a gated single
   commit: `d1ef290` mockIPC dashboard test · `482988b` orphan dialog (B8) · `d9416ed` terminal title/bell →
   header · `47458ea` `Facade::load_project` · **`d497241` project-load UI (folder picker + "Open project" +
   `demo.rs` removed)** · **`45461d0` trust review (A6/A9: `requires_trust` + enriched `ConfigChanged` +
   `Facade::trust_command` + inline sidebar Trust + `TrustDialog`)**. `just lint && just test` green: **132**
   (Rust **103** / UI **29**). Locked decisions still hold (tests inline; 7 placeholder modules + 4 stub
   crates stay; stray root `package-lock.json` left — do not rm/gitignore/stage). Phase 5 stays **`Done —
   pending verify`** until the runtime/manual checks below are observed.

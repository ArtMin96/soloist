...
- **Settings build-out (11a/11b) IN PROGRESS — the generic base (I7s) + all six SHOWN global tabs' core behavior are
  ...
  one `update` primitive; 22 settings tests green; `just lint` exit 0. **Remaining:** the Tauri command adapter (next,
  via the `tauri-*` skills), the Settings window UI (via `/impeccable`) + Appearance→xterm restyle (I5) + Playwright,
  and all of per-project 11a (I7a–I7e + the C1 shared/local move). One pre-existing environmental red in `just test`
  (the I10 `crates/sys` shellenv capture times out — sandbox login shell ~6.8 s vs 3 s cap; orthogonal to settings).
  See the top Decisions entry + "Next session should start with" §0.
...
| 11b | Global settings (8-tab Settings window) on the reusable settings base | In progress | Decomposes I7. **Core behavior of all six SHOWN tabs DONE & green** (`feat/phase-11-settings-ui`): Appearance/Sidebar/Agents/Tools/Integrations data documents (`73ed5d7`) + the Hotkeys remappable registry (`fe57dca`), each a `#[serde(default)]` sub-document on the singleton `Settings` with a whole-tab `Facade` getter/setter (auto-save) routed through the one `update` primitive. **Remaining:** the Tauri command adapter + the Settings window UI (via `/impeccable`) + Appearance→xterm restyle + Playwright; per-tab notes in `plan/05` §12. **Notifications/Account tabs (I7l/I7m) were never opened in the source — contents are a pending owner decision, to be stubbed "to be defined", not built blind.** Plan: `plan/phases/phase-11b-global-settings.md`. |
...
docs, `c0de87c` base, `73ed5d7` data tabs, `fe57dca` hotkeys — see the top Decisions entry). The next steps, in order:
  ...
  **0a. The Tauri command adapter for settings (the `tauri-*` skills are MANDATORY here — §5; confirm each API/config
  against the official Tauri docs).** Add one `#[tauri::command]` per `Facade` settings method (already written):
  `mcp_tool_groups`/`set_mcp_tool_group`, `appearance`/`set_appearance`, `sidebar_settings`/`set_sidebar_settings`,
  `agent_settings`/`set_agent_settings`, `tool_defaults`/`set_tool_defaults`, `integration_settings`/
  `set_integration_settings`, `hotkeys`/`remap_hotkey`/`disable_hotkey`/`reset_hotkey`/`reset_all_hotkeys`. Register them
  in the app's `invoke_handler`, add the capability/ACL entries (`tauri-capabilities`), and mirror the domain types +
  command names in **one** `domain.ts` + `api.ts` (the only sanctioned Rust↔TS duplication). Ends green (compiles + `tsc`
  + dep-direction + capabilities check). **This is the clean next slice — deterministic, no design work.**

  **0b. The Settings window UI — drive through `/impeccable` (MANDATORY, §5); `PRODUCT.md`/`DESIGN.md` are the design
  source of truth.** A `SettingsWindow` shell + a tab strip + one small presentational panel per tab over a projected
  read-model (no business logic in components, §15–16). Appearance theme + every terminal typography value must restyle
  the app tokens AND the **xterm.js** renderer immediately (I5) — map each closed enum to its CSS/xterm value in one
  `lib/` place. Hotkeys panel: search + per-row edit + hover-to-disable + Reset-all, scope badges, conflict highlight
  (the core already computes `conflicts()`). Integrations: reuse `McpToolGroups` (the served-surface change is G10, already
  wired) + the HTTP endpoint list (Phase 10). **Stub Notifications + Account (I7l/I7m) with a visible "to be defined"
  state — never invent fields** (decide with the owner first). Playwright e2e per the phase-11b test plan.

  **0c. Per-project settings (11a, I7a–I7e) — not started.** `ProjectSettings` via `SettingsStore<ProjectId,
  ProjectSettings>` (a new `project_settings` SQLite table + migration v10 + `project_settings_repo` on `CorePorts`); the
  C1 shared/local write path (a local command is NEVER written to `solo.yml` — assert the file bytes in a test); the
  project settings page + "Add command" modal + the Make-local move.

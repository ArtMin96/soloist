...
- **CHILD-AGENT LIFECYCLE — a worker reports to its lead, and may close itself — `Done — pending verify` (2026-07-31, branch `feat/child-agent-lifecycle`, 9 commits, not merged).** The owner reported that a spawned worker neither reported its result to its lead nor closed itself when done. A review of the whole `spawn_agent` path established that **neither behaviour was ever built** — no channel carried a worker's *content* to its lead (the only autonomous PTY write is a fired timer, and a timer always delivers to the process that armed it), and nothing removed a process from the registry on exit (`remove_returning_handle` has no call site in the exit path) — and found a third thing that **was** built and **was** broken: the delegation loop's "worker finished" signal is three seconds of terminal quiet, which a worker that is still booting satisfies. The review's own reproduction watched Soloist tell a lead *"all 1 watched agents are idle"* about a worker still rendering `connecting…` that had never been given a task. **The review report is at `.scratch/spawn-agent-review/REPORT.md` but is UNTRACKED** (`.scratch/` was removed by owner decision 2026-07-27 and is not wanted back), so it will not travel with the branch — the substance is recorded here instead.

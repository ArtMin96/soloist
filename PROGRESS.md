...
| 10 | HTTP API & CLI (`127.0.0.1:24678` + `soloist` CLI) | **In progress** | **Slice 1 (H2 + H1 transport/CORS) landed (2026-06-24):** `crates/httpapi` `axum` server (core-only) on `127.0.0.1:24678` with auto-fallback + a runtime port file (`ipc::http`), localhost-only `tower-http` CORS, and the 5 read endpoints (`/health`,`/status`,`/processes`,`/processes/{id}/ports`,`/projects`) — each one thin handler over one `Facade` method, reusing `ProcessView`/`ProjectView`. Runs in-process as a supervised task behind a new `http` Cargo feature (removable both ways). App now manages `Arc<Facade>` so the core-only server shares the one core. Gate green: Rust 549 (+8) / 3 ignored / UI 78; `just lint` + `just test` + 3 feature-matrix checks exit 0. **Remaining:** H1 auth header on mutations + H3 (mutation endpoints), H4 (`soloist` CLI), docs/acceptance. |

...
> worker closing when done — had **never been built**, and that the delegation loop's completion signal
> (three seconds of terminal quiet) fired on workers that had not started. Both are built now, plus the
> O13 spawn preamble for `spawn_agent`. **Nothing on the branch has been run against a live build** — every
> reproduction in the review was taken against the *installed* v0.11.0, i.e. the code before these fixes.
> **The idle fix is partial and the owner took a decision on the remainder: completion is explicit, terminal
> quiet is not a completion signal.** The headless gates are **independently verified green at `af73a39`**
> (Rust 1365 passed / 0 failed over three runs, UI 903, `just lint` exit 0) — but green gates are not a runtime
> pass, and **one test was proven by mutation not to test what its name claims**, so `report_to_lead`'s routing
> is untested. The preamble extension has since landed. See the entry below.
>
...
- **CHILD-AGENT LIFECYCLE — a worker reports to its lead, and may close itself — `Done — pending verify` (2026-07-31, branch `feat/child-agent-lifecycle`, 9 commits, not merged).** The owner reported that a spawned worker neither reported its result to its lead nor closed itself when done. A review of the whole `spawn_agent` path established that **neither behaviour was ever built** — no channel carried a worker's *content* to its lead (the only autonomous PTY write is a fired timer, and a timer always delivers to the process that armed it), and nothing removed a process from the registry on exit (`remove_returning_handle` has no call site in the exit path) — and found a third thing that **was** built and **was** broken: the delegation loop's "worker finished" signal is three seconds of terminal quiet, which a worker that is still booting satisfies. The review's own reproduction watched Soloist tell a lead *"all 1 watched agents are idle"* about a worker still rendering `connecting…` that had never been given a task. **The full review, with its verbatim reproductions, is tracked at `docs/research/child-agent-flow-review.md`** (it was first written under `.scratch/`, which the owner removed by decision 2026-07-27; note `CLAUDE.md` still names `.scratch/<feature>/` as the issue-tracker home and contradicts that decision). The substance is also recorded here, so this entry stands alone.
  - **The nine commits.** `5744c0f` the sidebar nests a worker under a lead of another kind (it resolved lineage only *within* one kind bucket, so an agent spawned from a Terminal rendered flat while the orchestration pane nested it — the two surfaces disagreed about one tree); `f4c902a` a fire-when-idle wait ends when a watched worker **exits** (a worker that exited while last classified `Working` never satisfied the quorum and waited out the 3600 s `DEFAULT_IDLE_MAX_WAIT` backstop); `6a59b67` a starting-up worker no longer reads as a finished one (**PARTIAL — see below**); `efeab90` lineage is recorded from whichever authenticated fact names the caller (binding first, then the peer process group), closing a gate/record asymmetry where a lead identifiable enough to be *gated* as a worker still recorded **no parent**, so its workers rendered as siblings; `bbe0109` a refused MCP bind is recorded on the session, reported by `whoami` as a field beside `origin`, written once to stderr by the client (deduplicated), and retryable through a new `bind_session_process` MCP tool routed through the same façade gate; `a208e60` opt-in `close_when_done` on `spawn_agent`; `5c7004c` the **O13** spawn orchestration-context preamble; `efb2958` the new `report_to_lead` MCP tool; `1790bd8` tool, guide and preamble wording corrected — going quiet is not the same as finishing.
  - **`report_to_lead` — the completion channel.** A worker's result is delivered to its lead as a fresh **submitted** turn, in the same header-then-body shape a fired timer wakes an agent with, so a wake reads the same whatever produced it. The target is **resolved from recorded lineage and cannot be named by the caller** — a caller with no lead is refused rather than defaulted onto someone's terminal — which is strictly narrower than the `send_input` the same worker may already call on any process in its project, so this adds an *address*, not reach, and `send_input`'s gate was left exactly as it was. The report is capped like a timer body and delivered non-blocking, so one deaf lead cannot stall a worker.
  - **`close_when_done` — opt-in, off unless asked for.** A worker armed with it is closed (reaped, forgotten, terminal buffers freed) on the transition that ends its run, by a reactor over the process event stream alongside the crash policy. **A failed run is still a finished one**, so a crashed worker is closed too; a caller that wants the output simply does not arm it. Arming happens *before* the launch so an immediate exit is still caught, and a resting process that has yet to start is never mistaken for a finished one. The default stays "the user can read the worker's scrollback" — a blanket auto-close would destroy output the user may want.
  - **O13 is delivered for `spawn_agent` ONLY, not `spawn_process`** (which stays gated on the O9 arbitrary-spawn trust work). The scoped spawn opens the worker's CLI on a preamble naming the lead that spawned it, the project its tools act on, and the same capability overview `help` serves — so a worker works with no skills loaded. **Only the scoped spawn briefs**: an agent the user opens from the dashboard is theirs, not a worker. The worker's own process id is *not* inlined (it is minted by the registration the command line is built for, and already arrives as `SOLOIST_PROCESS_ID` with the session bound), so the preamble points at `whoami` rather than carrying a second copy that could disagree. How a CLI takes a first turn is per-provider, so it joined resume as a Strategy: Claude and Codex carry it on their documented interactive prompt argument, a Generic tool declares its own convention through **`prompt_mode` — until now dead state with zero read sites workspace-wide** — and a provider documenting neither launches exactly as before, because an unverified flag is not a worse briefing but a CLI that never starts. A resume never carries the turn.
  - **`6a59b67` IS A PARTIAL FIX — the noisy-boot case still reproduces.** A worker with a **silent** boot no longer reads as finished: an agent's activity is now tracked only from the point it demonstrably began, so the quiet *preceding* first activity is unclassified, which a fire-when-idle wait already treats as not idle, while what a surface renders still reports `Idle` (which is what the terminal shows). **A worker with a NOISY boot — a TUI banner prints, then goes quiet — still reads as finished ~4 s after launch**, because `output_seq` is a cumulative byte count and the output-delta heuristic cannot tell quiet-*before*-work from quiet-*after*-work. Measured on the live app, not inferred: a fresh `claude` worker's raw output was **byte-identical by SHA-256 for ~6 s, changed once, then byte-identical for a further ~8 s** — the spinner is a static frame, not an animation, so a booting agent really does produce quiet windows far past the 3-sample threshold. An independent run polling raw-output length reproduced the same shape.
  - **DECISION (owner, 2026-07-31) — completion is explicit; terminal quiet is not a completion signal.** A worker counts as finished when it **exits**, calls **`report_to_lead`**, or **completes its todo** — nothing derived from terminal output stands in for the worker saying it is done, because a worker that has finished and a worker that is still thinking are indistinguishable from outside. `timer_fire_when_idle_*` remains a convenience for "the watched processes went quiet — that is where to look", **not** a completion guarantee, and `1790bd8` rewrote the three surfaces that implied otherwise (the guide's timers topic, both fire-when-idle tool descriptions, and a preamble that suggested reporting rather than requiring it). **No settle-time constant was added — inventing one was explicitly declined**, since any threshold would encode a guess about boot duration that the byte-count heuristic cannot make true. Recorded in `KNOWN-DIVERGENCES.md` **D-35**, `plan/05` §12, and the O7/O8 rows.
  - **Docs updated this session.** `plan/02` rows **E7** (the loop has an explicit completion signal), **F4** (`bind_session_process` is now a real tool — the row had named one that did not exist), **F11** (`report_to_lead`, `close_when_done`), **I14** + **O3** (nesting crosses kind buckets; lineage from an unbound-but-identifiable caller), **O7**/**O8** (what fire-when-idle actually fires on). **O13 was already amended by the branch's own commits and was correct at this commit — verified, not re-edited — but in-flight work extends the preamble to five more providers and will change it again (see below).** `KNOWN-DIVERGENCES.md`: **D-15**'s "there is no manual bind tool" half is **reversed** (the whoami-omits-OS-pid half stands), which also resolves a pre-existing doc-vs-doc contradiction in `plan/02` F4's favour; **D-5** amended for what the quiet window still governs versus what it no longer counts for; new **D-35** for explicit completion. Three `plan/05` §12 rows. The MCP tool surface is **95** with every feature group enabled (`EXPECTED_TOOL_SURFACE`, `crates/mcp/src/server_tests.rs`), including the two added here.
  - **Evidence — INDEPENDENTLY VERIFIED at `af73a39` (an independent agent ran every gate itself; the numbers below are not the implementers' self-reports).** `cargo test --workspace --no-fail-fast` **1365 passed / 0 failed / 0 ignored, exit 0, on three consecutive runs** over 31 test targets, on an idle machine. UI `pnpm vitest run` under Node 22: **132 files / 903 tests passed, exit 0**. **`just lint` exit 0** end to end (`cargo fmt --check`, clippy `-D warnings`, tsc, ESLint, Prettier, `check-core-deps.sh`, `check-core-cycles.sh`, `check-file-size.sh`, schema drift), printing only the advisory non-gating file-size warning — **17 files over the 400-line smell, an identical list on `main` and on branch**, so nothing new crossed it (`facade.rs` grew 447 → 501 having already been over). Incidentally disproving an assumption carried in earlier entries: **`pnpm -C` did not break under pnpm 11 here.** New Rust tests include `quiet_before_an_agent_has_worked_is_not_observed_as_a_finished_turn`, `reset_drops_the_turn_observed_before_the_agent_stopped`, `a_watched_worker_that_has_not_begun_a_turn_does_not_fire_the_timer`, `a_watched_process_that_exits_while_working_fires_the_timer`, `fire_when_idle_reports_the_same_state_in_the_timer_it_returns`, `a_lead_that_never_bound_still_has_its_worker_nest_under_it`, `a_caller_soloist_cannot_name_spawns_a_root`, ~~`a_workers_report_reaches_its_lead_as_a_submitted_turn`~~ (**struck — it does not test what it names; see the warning below**), `a_caller_no_agent_spawned_has_no_lead_to_report_to`, `a_report_to_a_departed_lead_is_refused`, `an_oversized_report_is_refused_and_the_cap_itself_is_allowed`, `an_armed_process_is_closed_when_its_run_ends`, `an_armed_process_that_crashes_is_closed_too`, `an_unarmed_process_rests_in_the_registry_after_it_exits`, `a_lagged_reactor_still_closes_a_finished_process`, `a_refused_bind_is_reported_by_whoami`, `a_bind_that_succeeds_clears_an_earlier_refusal`, `a_spawned_worker_opens_on_its_orchestration_context`, `an_opening_turn_reaches_only_the_launch_that_asked_for_one`, `a_worker_is_told_that_finishing_quietly_signals_nothing`, and the five `initial_prompt` per-provider tests; frontend `nests a worker under a lead of another kind, in the lead's group` and `jumps to a worker nested under a lead of another kind`.
  - **NOT VERIFIED AGAINST A RUNNING BUILD — this is why the status is `Done — pending verify`, not `Verified`.** Every reproduction in the review was run against the **installed v0.11.0**, i.e. the code *before* these fixes; nothing on this branch has been exercised in a live app. Unproven at runtime: a real worker receiving the preamble as its opening turn; a real `report_to_lead` landing as a submitted turn on a real lead's terminal; `close_when_done` removing a real worker's row; a refused bind surfacing through `whoami` and being retried with `bind_session_process`; and the sidebar nesting a worker under a Terminal lead in a real window.
  - **The pty red is LOAD-SENSITIVE, not deterministic — and the mechanism is now known.** `crates/pty/tests/integration.rs:658 create_terminal_runs_an_interactive_shell_in_the_project_dir` **passes idle and fails under CPU contention, on `main` and on this branch alike.** Measured on this host by the independent verifier, not inherited from earlier entries: under load **5/5 FAIL on an `origin/main` worktree and 5/5 FAIL on the branch** with an identical panic; idle, **5/5 PASS on the branch**; and the mechanism was reproduced directly — with 16 busy-loops running (loadavg ~6–17) **3/3 FAIL**, idle **5/5 PASS**, same commit and same code both times. The A/B used a throwaway `git worktree` at `origin/main` (the primary tree was never stashed or switched). **The cause is the test, not the product:** it spawns a real interactive `$SHELL` (`/usr/bin/zsh`) and writes the typed line before the shell is ready to read it, so the panic shows the raw line echoed by the tty rather than its expansion. Not caused by this branch, which adds no PTY behaviour (`a208e60`/`5c7004c` touch `crates/pty/tests/*` only to pass the new spawn arguments). **Correcting the record: earlier entries called this a "flake" and one report called it deterministic — it is neither; it is a load-sensitive race in the test, and it is fixable by making the test wait for readiness.**
  - **A second pre-existing flake, newly identified: `crates/app/ui/src/App.test.tsx`.** Across 8 full-suite runs on the branch, **7 passed / 1 failed**; across 4 runs on an `origin/main` worktree, **2 passed / 2 failed**. The same test fails on both — `selects a process and opens its terminal pane` — timeout-shaped at 2.0–5.7 s. Not branch-caused, not previously recorded, not fixed here.
  - **⚠️ ONE TEST IS NOT EVIDENCE FOR WHAT ITS NAME CLAIMS — owed follow-up.** `a_workers_report_reaches_its_lead_as_a_submitted_turn` does **not** verify that a report reaches *the lead*. Proven by mutation: `report_to_lead` was changed to deliver to the **worker** instead of the lead and **the whole workspace stayed green**. The cause is the fixture, not the assertion style — `FakeSpawner::records_input()` (`crates/core/src/testing/spawner.rs:211`) hands **one shared buffer to every spawned child**, so the helper's `lead_input` (`crates/core/src/facade/scoped_tests.rs:541`) cannot tell which process received the write. **The production code is correct** (the target is resolved from lineage and cannot be named by the caller); what is missing is a test that would fail if it were not. This is exactly the failure mode `CLAUDE.md` §15 warns about — a test that cannot fail for the right reason — so the routing of `report_to_lead` should be read as **untested**, not as covered, until the fake gives each child its own buffer.
  - **The preamble extension LANDED after this entry was first written — `af73a39`, and the O13 row is current again.** Of the five remaining providers, **two gained an opening turn and three deliberately did not**, each decided from fetched vendor documentation rather than a guess: **Gemini** takes `--prompt-interactive` (which runs the turn and *stays* in the session, unlike `--prompt`, which answers and exits) and **OpenCode** takes `--prompt` on its terminal UI (distinct from the non-interactive `opencode run`); both bind the prompt as a flag's value at the end of the command line, where Claude and Codex take it as a trailing positional. **Amp, Copilot and Kimi launch plainly** — Copilot's and Kimi's only prompt flag runs the turn non-interactively and *exits*, and Amp accepts an opening message only on piped stdin, which a PTY launch never supplies, so each would end the very worker its lead still needs to talk to. **`spawn_process` remains out of scope**, gated on the O9 arbitrary-spawn trust work. The branch is now **13 commits** (the nine above, this ledger entry, the tracked review, its ledger pointer, and `af73a39`).
  - **STILL NOT DONE — the live verification.** No part of this work has been exercised against a running build; that pass is what would move it off `Done — pending verify`, and it is the first item below.
  - **Open threads.** (1) **The noisy-boot idle gap above** — the owner's decision makes it non-blocking for delegation, but `timer_fire_when_idle_*` can still fire early on a booting worker, so anything treating it as completion is still wrong. (2) **`ScopedFacade::timer_list` returns un-enriched timer views** — `crates/app/src/ipc_server/dispatch.rs:309` → `facade/coordination.rs:384` returns `timers.list(owner)` with no `report_idle` pass, so an MCP caller reads `waiting_on: []` / `already_idle: false` (the aggregate defaults) while the orchestration snapshot enriches the same views (`facade/orchestration.rs:91-98`). `6a59b67` fixed the *arm* reply; the list path was flagged as out of scope and left. (3) **Provider coverage is uneven** (code-read, not reproduced): `looks_like_permission_prompt` has one call site, inside `OutputDelta`, so a Codex/Amp worker blocked on a permission prompt reads as `Idle` and `AgentPermission` can never fire for those providers. (4) `AgentLineage` is in-memory only and reachable outside the Tauri UI **only** through `report_to_lead`'s implicit resolution — no tool exposes the tree an agent is part of.
  - **Next session should start with:** **the live verification, then the untested `report_to_lead` routing.** The headless gates are green and independently verified at `af73a39`, and the preamble extension has landed, so what remains is the part no test can reach. Launch this branch against a real app with **`just dev-alongside`** (an installed Soloist is running; plain `just dev` would hijack its MCP socket and exit without opening a dev window) and walking the loop end to end — spawn a worker with `close_when_done: true`, confirm it opens on the orchestration preamble, have it call `report_to_lead`, confirm the report lands on the lead's terminal as a submitted turn and that the worker's row leaves the sidebar when its run ends; then spawn one from a Terminal lead and confirm the sidebar nests it. Then **give `FakeSpawner` a per-child input buffer so `a_workers_report_reaches_its_lead_as_a_submitted_turn` can fail when the report goes to the wrong process** — today it cannot, so that routing is untested. Then decide whether the noisy-boot idle gap needs anything further now that completion is explicit, and whether `ScopedFacade::timer_list`'s un-enriched views are worth fixing. The two pre-existing flakes (`crates/pty` under load, `App.test.tsx`) are diagnosed above and worth fixing at their cause, but neither belongs to this work.

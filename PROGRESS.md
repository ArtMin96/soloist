...
- **R8 — MCP tool-router split landed (2026-06-23, branch `refactor/mcp-tool-router-split` off `main` `84230ca`):**
  ...
  for 31 thin forwarders; YAGNI). **Pure code movement — tool surface byte-identical** (same `#[tool]` +
  `Parameters<Arg>`), asserted by the unchanged `server_tests.rs`. `server.rs` 546 → **46** lines; each category
  file ≤151. Gate green, **no test-count change**: core 254 / ipc 13 / app 28 / mcp 31 / store 15 / sys 5 (+10
  integration) / pty 1 (+12 integration, +3 soak ign) / UI 78; clippy `-D warnings`, fmt, tsc, eslint, prettier,
  dep-direction, file-size (now only the pre-existing `supervisor.rs` 401), `cargo check -p soloist-app
  --no-default-features` all green. **Not pushed / no PR yet** — stopped for review per the user's directive.
...
**A. Phase 8 is `Done — pending verify` (all v1 F-rows code-complete, all PRs merged to `main`) — push the R8
branch + run the user-only runtime acceptance walk to flip Phase 8 `Verified`, then start Phase 9.**
Every Phase-8 PR (#16–#20) is **merged to `main`** (HEAD `84230ca`; PR #20 merged `2026-06-23T17:40Z`). The full
v1 F-set — **F1, F3–F11, F13** — is code-complete and merged. **R8 (the MCP tool-router split)** also landed this
session on branch **`refactor/mcp-tool-router-split`** (off `84230ca`, commits `150cd88` refactor + the docs
commit) — **not pushed / no PR yet** (stopped for review per the user's directive). **Next, in order:**
1. **Push `refactor/mcp-tool-router-split` + open its PR (ArtMin96), review, merge to `main`.** Pure code movement
   (server.rs 546 → 46; seven `tools/<cat>.rs` sub-routers composed via `ToolRouter` `Add`); tool surface
   byte-identical, gate green, no test-count change. See the top Decisions entry.
...
3. **Then Phase 9 (Coordination layer, v1)** — scratchpads/todos/timers/leases/kv; the metaharness centerpiece,
   ...
   kv); invoke `mcp-builder` if it gets installed, else the official MCP docs + `rmcp` via context7. **Its ~50 new
   MCP tools now land in new `crates/mcp/src/tools/<category>.rs` sub-routers** (coordination/scratchpad/todo/
   timer/lock/kv), added to `SoloistMcp::new`'s `Add` composition — never a flat block (R8 set this up).

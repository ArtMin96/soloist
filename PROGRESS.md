...
- **Overall:** **Phase 5 (Dashboard UI) — `Done — pending verify`.** Latest: a third 2026-06-19 session
  fixed the silent empty-project-load (`72b526e`; gate **134 — Rust 104 / UI 30**), the user **runtime-
  confirmed** project-load via the picker, and **decided to pull A10 (command auto-detection) into v1**
  (auto-create a `solo.yml` from detected commands on opening a config-less folder, with friendly copy) —
  **not yet built**. See the top "third session" entry under Decisions. The rest of this block describes
...
### Phase-5 follow-up — third session (2026-06-19): project-load empty-load fix + A10 pulled into v1
...
0. **BUILD A10 — command auto-detection (user decision, this session; the immediate next work).** When a
   picked folder has **no `solo.yml`**, auto-create one from **auto-detected commands** (package.json
   scripts, Procfile, Makefile/justfile, Cargo, go.mod, docker-compose, … per plan/05 §9), then show a
   **friendly, non-developer confirmation** naming the file/folder. Architecture mandate (user): single
   trusted source, no duplicates, clear separation → a dedicated detection+writer domain in C1
   (`core/config/`), **Registry/Strategy** (one `Detector` per ecosystem, registered once), single-sourced
   through the `SoloYml`/`ProcessSpec` model (writer serializes via the model + a hand-written header),
   `ProjectLoad` gains `created`, one fact flowing core → command → `api.ts` → `useProjects` → `EmptyState`.
   First update **`plan/02` (A10 → v1)** + `plan/05 §9` cross-ref + this ledger. Build in gated,
   one-concern commits. Baseline before starting: `git log` newest = `72b526e`; gate **134 (Rust 104 /
   UI 30)**. A SESSION-START prompt for this was handed to the user verbatim — use it. **Also finish the
   deferred adversarial review** of `d497241`+`45461d0`+`72b526e` (REVIEW-PROMPT.md; open finding:
   `useTrust` optimistically drops/closes a review before `configTrust` resolves). Locked decisions hold
   (tests inline; 7 placeholder modules + 4 stub crates stay; **leave** the stray root `package-lock.json`
   AND the new 0-byte root `solo.yml` — do not rm/gitignore/stage). Phase 5 stays **`Done — pending
   verify`** until the runtime/manual checks below are observed.
1. **Confirm Phase 5 at runtime, then flip it to `Verified` (user-only — needs a desktop `just dev`,
   host `DISPLAY=:0`).** Observe, with evidence: (a) launch with no project → empty state; **Open project**
   → pick a folder with a `solo.yml` → its stack populates (project-load); (b) an untrusted command shows
   Start disabled + a **Trust** affordance → click Trust → it becomes startable (A6, first-open); (c) a
   leftover process group surfaces the **orphan dialog** (B8). **A9 end-to-end** (the trust dialog popping on
   a *live* `solo.yml` edit) is **gated on the Phase-6 file watcher** — it is emit-tested now, but no runtime
   trigger exists until the watcher lands; verify it during Phase 6. Once (a)–(c) are observed, Phase 5 →
   `Verified` (also flips the long-open Phase 1 in-GUI click, same run).
2. **Begin Phase 6 — Monitoring, restart (10/60s), file-watch, notifications.** Read
   `plan/phases/phase-06-*.md` + its matrix rows first. Key pieces: crash auto-restart capped **10/60s →
   `RestartExhausted`**; the **live `notify` file watcher** that drives `ConfigEngine::sync` through the
   `Debouncer` (single writer per project, off-thread `spawn_blocking` — see the open thread) and glob
   file-watch restart (D6); CPU/mem sampling (C5); crash/attention notifications (C7). **The nightly soak
   test starts running from Phase 6.** Wiring the watcher is what finally makes the **A9** trust dialog fire
   on a real edit at runtime — verify it then.

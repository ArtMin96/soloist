...
The leases slice is gate-green (core 266 / store 23 / ipc 14 / app 30 / mcp 37 / sys 5 +10 / pty 1 +12 +3 ign /
UI 78; `just lint` + `just test` + `cargo check -p soloist-app --no-default-features` all pass). **In order:**
1. **Review + merge the leases PR** (`feat/phase-9-leases`). Pure additive vertical slice on the established
   ports/adapters structure; gate green.

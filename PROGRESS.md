...
- **Overall:** **Phase 7 (Agents & idle detection) — all v1 rows (E1–E5, E8) code-complete; `Done — pending
...
- **Active phase:** **Phase 7 (Agents & Idle Detection, C4)** — `In progress` (started 2026-06-22 per the
  ...
  code-complete**; **E6** (summarization) `later` + OFF by default, **E7** in P9. Gate **green: Rust core 201 /
  store 15 / sys 5 (+10 integration) / pty 11 (+3 ignored) / UI 77**; `just lint` + `just test` clean. Runtime
  verify (idle FSM tracking a real agent via `just dev`) is owed (user-only). See the top Decisions entry +
  "Next session should start with" item A.
...
**A. (Phase 7) E5 is DONE — runtime-verify it, then Phase 7 → `Verified`, then Phase 8.** All Phase-7 v1
rows (E1–E5, E8) are code-complete on branch **`feat/phase-7-idle-detection`** (off `main`; 2 feature commits +
docs; **not pushed / no PR** — awaiting the user's call). Gate **green: Rust core 201 / store 15 / sys 5 (+10
integration) / pty 11 (+3 ignored) / UI 77**; `just lint` + `just test` clean. **Runtime acceptance walk
(user-only, `just dev`, desktop):** launch a real agent (e.g. `claude`) via the `Cmd/Ctrl+T` picker → its row's
left indicator shows **Working ▶** while it streams, settles to **Idle ○** when it goes quiet, and shows
**Permission ◆** (gold) when it prompts for approval (which also fires a desktop **toast**); the terminal-header
indicator mirrors it with the inline label; hovering the dense-row dot shows the label tooltip; a stopped agent
falls back to its **ProcStatus** dot. Also the **E8** smoke: a fresh `claude` with no stored creds completes its
native login in its Soloist terminal. Once observed, Phase 7 → `Verified`. **Then Phase 8 (MCP server core —
`soloist-mcp` stdio, scope+identity, tools; high-risk):** invoke the `mcp-builder` skill + the MCP docs
(`code.claude.com/llms.txt`) first; the future `spawn_agent` tool routes to the existing `Facade::launch_agent`.
**E6** (summarization) stays `later` + OFF by default; **E7** completes in P9. Idle-heuristic thresholds/cues are
a recorded gap (`KNOWN-DIVERGENCES.md` D-5 / plan/05 §12) — most likely thing to tune as real agent CLIs are
observed. (The **package-lock.json** gitignore question is still open — see Decisions.)

...
- **Overall:** **Phase 6 (Monitoring / self-healing) — `In progress`; Phase 5 (Dashboard UI) — `Done —
  pending verify`.** Newest (2026-06-20): **the OS-probe slice — D1 per-process CPU/mem + D2 port discovery**
  (gate **207 = Rust 166 / UI 41**). Two new **C5 monitoring domains** (`core/metrics/`, `core/portscan/`),
  each owning its own port; self-supervised `Clock`-driven samplers publishing `MetricsTick` / surfacing
  `ProcessView.ports`; the new **`crates/sys`** adapter (sysinfo + `/proc`). Commits `e0fa32e` (D1),
  `be1711a` (D2) on branch `feat/phase-6-monitoring`. The prior **crash auto-restart policy** (D4 + D11)
  merged as PR #7. **Next: D3 readiness** (`wait_for_port` + not-ready sub-state). See the top Decisions entry.
...
- **Not done / next:** **D3 readiness** (`Supervisor::wait_for_port(id, port, timeout)` polling the `PortProbe`
  on the `Clock`; a `ready` dimension on `ProcessView` + `ReadyStateChanged`, "Running but not Ready"
  sub-state) — the `PortProbe` it needs now exists. Then D6/D7 file-watch (flesh out `FileWatcher` + `notify`)
  and D8 notifications (`Notifier` + `notify-rust`), the nightly soak gate, and the UI surfacing of CPU%/RSS +
  ports + the "restarting (k/N)"/RestartExhausted/not-ready badges (phase Task 5/9, via `/impeccable`).
  **Next session should start with: D3 readiness**, building on the now-existing `PortProbe`/`PortScanner`.

...
**A. Phase 9 is IN PROGRESS — leases (G6), timers (G7–G9) AND scratchpads (G1/G2) landed. Next: todos G3–G5.**
PR #22 (leases) and PR #23 (timers) are **MERGED to `main`** (`e1435dc`). Scratchpads landed this session on branch
`feat/phase-9-scratchpads` (off `main` `e1435dc`, commit `6e4d758`; **PR pending — open it, then merge is the
user's call, do NOT self-merge**). Gate green (core 314 / store 46 / ipc 14 / app 30 / mcp 50 / sys 5 +10 / pty 1
+12 +3 ign / UI 78; `just lint` + `just test` + `cargo check -p soloist-app --no-default-features` all pass). **In
order:**
1. **Open + (on the user's go) merge the scratchpads PR** (`feat/phase-9-scratchpads`). Additive vertical on the
   established structure (C6 `Scratchpads` aggregate over a typed, validated `ScratchpadDoc` rendered to canonical
   Markdown + `ScratchpadRepo`/`NoopScratchpadRepo`; SQLite + migration **v6**; revision-guarded writes; 9 MCP
   tools; ipc arms). **Disciplined typed structure is a clean-room divergence (project-owner directive) —
   `KNOWN-DIVERGENCES.md` D-7 + `plan/05` §12.** Durable (survives restart, G11 — no reconcile clear). Also folded
   in two file-size tidies (`ipc/error.rs`, `facade/loops.rs`).
2. **Next slice — todos G3–G5**, carrying the **same disciplined-schema discipline** the owner asked for
   (`title` · `description`/path · `acceptance_criteria[]` · `risks[]` · `status` (closed enum) · `tags` ·
   **blockers** (a todo is gated until its blockers complete) · comments · **process-owned `todo_lock`/`_unlock`**
   that reuse the existing `LockReleaser` close hook — like leases): a `TodoRepo` over the store (migration **v7**),
   the `todo_*` tool family (`05` §7), revision-guarded like scratchpads where mutated concurrently. Then **kv G10**
   (project-scoped JSON, default off), then **E7** end-to-end (`spawn_agent` → assign todo → lock →
   `fire_when_idle_all` → integrate on wake). Each a thin handler → one `Facade` method → existing core behaviour;
   follow the SAME vertical the leases + timers + scratchpads slices set; consult the **MCP spec**
   (`modelcontextprotocol.io`, current rev **2025-11-25**) + `rmcp` via context7 (mcp-builder is not installed).
2a. **Scratchpad follow-ups (tracked, none G1/G2-blocking — pick up when convenient):** the free-form-oriented
   tools Solo lists (`_append`/`_edit`/`_append_section`/`_tail`/`_find`/`_clear`) need a disciplined design against
   the typed doc; cross-project `_transfer` introduces a cross-scope question; **file-io `_save_to_file`/
   `_load_from_file` needs a project-root-scoping security pass** before an MCP tool touches the host filesystem.
   All recorded in `KNOWN-DIVERGENCES` D-7 / `plan/05` §12 so nothing is lost.

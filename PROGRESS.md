...
- **F15 COMPLETE — prompt templates render, end to end — `Done — pending verify` (2026-07-20, branch `feat/prompt-template-render`, stacked on `fix/markdown-entity-corruption` / PR #79).** The vertical shipped 2026-07-18 stored templates and derived `{{placeholder}}` names that **nothing consumed**. It now substitutes values and delivers them by two routes. Fourteen tracked todos, each **mutation-verified** (break the fix, watch a test redden, restore byte-identical) per the CLAUDE.md §15 rule tightened during this work.
  - **Final gate, run with no agents active:** `cargo test --workspace` **31 suites / 1148 passed / 0 failed / 3 ignored** (from 1049 at the PR-#78 baseline); `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --check` clean; `check-core-deps.sh` framework-free; `check-core-cycles.sh` 140 edges no cycles; `pnpm test` **85 files / 459 passed**; `typecheck`, `lint`, `format:check` all clean. **`just e2e` was NOT run** — no template spec exists (defect #10, deferred to a follow-up todo now that there is finally UI to walk).
  ...
  - **Still open (deliberately, not forgotten):** the **owner's real-window acceptance walk** is what flips this to `Verified` — no live-app visual check was possible in this session. The running `soloist-mcp` is the **pre-change binary**, so `prompt_template_render` and the prompts primitive will not appear until the app is rebuilt and the server reconnected; **the `PromptTemplates` group also defaults OFF** and must be enabled in Settings → Integrations before an agent sees any of it. Follow-up todos: **34** (e2e spec — mutation-verified, needs pinned Node 24 from `e2e/.nvmrc`, never overlap runs) and **35** (two dead code paths found by the mutation pass: an unreachable `FromSqlConversionFailure` branch, and the one mutation of eighteen that could not be made to fail). `Cargo.lock` was left uncommitted — it was already modified before this work began and is not ours to claim.

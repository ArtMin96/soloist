...
> **ACTIVE PHASE: 11 (UX Polish & Execution Profiles) — STARTED. First slice landed: I10 env capture (`$SHELL -ilc env`, ~10-min cache, precedence process > captured > app). Phase 10 (HTTP API & CLI) stays `Done — pending verify` — it owes only the user-only runtime acceptance walk (see "Next session" A). Phase 9 (Coordination, C6) is `Verified`.**
...
- **Phase 11 STARTED — slice 1: I10 env capture landed (2026-06-24).** Managed processes now launch with the
  ...
  `--features mcp`) all build. **Not yet committed** — working tree only; commit on a `feat/phase-11-*` branch, the
  user merges. **I10's matrix Verify** ("Version-manager PATH visible") is met headlessly by the real-shell
...
- **Not committed yet** (working tree). Next: commit on a `feat/phase-11-*` branch; the user merges (no self-merge).
...
**0. Phase 11 (UX Polish & Execution Profiles) is the ACTIVE phase. I10 env capture LANDED (working tree — not yet
committed).** First: **commit I10 on a `feat/phase-11-*` branch and open a PR for the user to merge** (no self-merge).
Gate is green (`just lint` exit 0; `just test` exit 0 — **Rust 593 / 3 ignored, UI 78**; feature matrix builds).
Then pick the next v1 slice. **v1 rows remaining:** `I1` drag-reorder, `I2` command palette (`Ctrl+K`), `I5`
light/dark/system themes (app + xterm), `I6` keyboard-first nav, `I7` settings screen (Appearance/Terminal/
Notifications/Sidebar/Agents/Tools/**MCP**/Hotkeys — the MCP tab carries the **G10 Key-Value default-OFF** per-group
toggle, the cross-phase carry-in), `I9` open-in-editor. **`later` rows I3/I4/I8/I11–I14 are tracked, NOT v1 — do not
gold-plate.** Most remaining rows are **UI → drive through `/impeccable`; DESIGN.md is deferred, so CONFIRM visual
specifics with the user before building** (CLAUDE.md §5). Two are backend-first and unblocked: the **`SettingsRepo`
(migration v9) + per-group MCP tool-enablement Registry behind the one `Facade`** (the foundation `I5`/`I7`/`G10`
persistence needs — `G10` kv defaults OFF, plan/05 §7; never a per-adapter branch, plan/06 §4), and parts of `I9`.
**I10 done:** `core::shellenv` (port + pure resolver) + `soloist_sys::CommandShellEnvProbe` + composition-root
wiring; decisions in `plan/05` §12; Verify met headlessly (real-shell integration + through-the-`Supervisor` wiring
test). A runtime spot-check (a launched process's `$PATH` shows a version manager) is a nice-to-have for the Phase-11
acceptance walk.

**A. Phase 10 — HTTP API & CLI — `Done — pending verify`; owes ONLY the user-only runtime acceptance walk.** PR #26
is **MERGED to `main`** (merge `7db4004`, incl. review-cleanup `a83ac19`); the merge was reconciled this session and
the gate re-confirmed on `main` (Rust 580 / 3 ignored, UI 78, `just lint` exit 0). All H1–H4 are ✅ in `plan/02` and
the headless tests meet every Verify, so the walk is **confirmation, not new evidence**. **Run the walk (item 4(b)
below); on a green walk + the user's confirmation, flip Phase 10 → `Verified`.** Phase 9 is `Verified` (PR #25 merged

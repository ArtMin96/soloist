...
**A. (Phase 7) E5 is DONE + MERGED — runtime-verify it, flip Phase 7 → `Verified`, then Phase 8.**
All Phase-7 v1 rows (E1–E5, E8) are code-complete and **merged to `main`** via **PR #15** (`b95dc6a`; branch
deleted). PR #15 had a full review + the review-fixes (commit **`8763948`**): Permission is reported only once
output settles (no sticky false-positive), the idle sampler reads a cheap terminal tail instead of cloning the
whole scrollback, and it skips the supervisor snapshot when no agents are tracked. Gate **green: Rust core 202 /
store 15 / sys 5 (+10 integration) / pty 11 (+3 ignored) / UI 77**; `just lint` + `just test` clean. **Runtime acceptance walk

...
> **ACTIVE PHASE: 9 (Coordination, C6) — `Done — pending verify`, NOT yet `Verified`. Do NOT start Phase 10.**
> The immediate next action is **the user merging PR #25** (todos + kv + E7), after which Phase 9 flips to
> `Verified`. **One intentional cross-phase deferral remains:** the **Key-Value "default off" tool gate (G10 /
> phase-09 Task 6)** is **Phase 11** work — it needs the per-group enablement toggle/settings infrastructure that
> Phase 11 builds (the phase-09 task says "per-group settings (Phase 11 surfaces toggles)"; plan/05 §7 lists
> Key-Value as a feature-group toggle that "defaults OFF"). It is **not** implemented in Phase 9 and **must not** be:
> "default off" with no enable path would make kv unusable and fail G10's own round-trip Verify. Phase 9 delivers the
> kv aggregate + tools (always-on for now) + persistence; Phase 11 adds the toggle that defaults it off. The phase-09
> **Verify contract** (the gating checks, CLAUDE.md §7) for G1–G11 + E7 all pass; this deferral is tracked, not a
> regression. See "Next session should start with" → A.
...
**A. Phase 9 is `Done — pending verify` — the v1 coordination Verify checks (G1–G11 + E7) all pass. DO NOT start
Phase 10 yet** (the immediate next action is merging PR #25; Phase 9 only flips to `Verified` after that). PR #22
(leases), #23 (timers), and **#24 (scratchpads)** are **MERGED to `main`** (`67787bc`). Todos (G3–G5) + kv (G10) + E7
are on branch `feat/phase-9-todos` = **PR #25**. **E7 — the end-to-end orchestration test — is DONE**
(`crates/pty/tests/orchestration.rs`; real PTY + real idle sampler + real scheduler; mutation-verified; stable under
full-suite load). Gate green (Rust **541** / 3 ignored / UI 78; `just lint` + `just test` + `--no-default-features`
all exit 0). **In order:**
1. **Merge PR #25 on the user's go (do NOT self-merge), then flip Phase 9 → `Verified`.** PR #25
   (`feat/phase-9-todos`) carries everything: `Todos` aggregate (G3–G5), `Kv` aggregate (G10) + its persistence test,

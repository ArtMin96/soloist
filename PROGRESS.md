...
- **Settings build-out (11a/11b) IN PROGRESS — the generic base (I7s) + all six SHOWN global tabs' core behavior are
  ...
  **app-defined commands need no capability/ACL entries** (the existing 14 commands prove it; the ACL gates only
  plugin/core commands), so none were added. `just lint` exit 0; UI vitest 78; Rust suite green. **Remaining:** the
  Settings window UI (slice 0b, via `/impeccable` against the approved `DESIGN.md`) + Appearance→xterm restyle (I5) +
  Playwright, and all of per-project 11a (slice 0c — I7a–I7e + the C1 shared/local move). One pre-existing environmental
  red in `just test` (the I10 `crates/sys` shellenv capture times out — sandbox login shell ~6.8 s vs 3 s cap;
  orthogonal to settings). See the top Decisions entry + "Next session should start with" §0.

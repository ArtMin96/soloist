...
- **Overall:** **Phase 6 (Monitoring / self-healing) — `In progress`; Phase 5 (Dashboard UI) — `Done —
  pending verify`.** Newest (2026-06-20): **the D6/D7 file-watch restart — CORE POLICY ONLY** (the real
...
- **Active phase:** **Phase 7 (Agents & Idle Detection, C4)** — `In progress` (started 2026-06-22 per the
  ...
  commands + the `AgentTool`/`AgentKind`/`DetectedTool` TS mirror. **Remaining v1:** the E4 **launch picker
  UI** (`Cmd/Ctrl+T` + "agent with flags") — **awaiting the user's visual sign-off**, then **E5** (idle FSM);
...
- **Not done / next:** the E4 **launch picker** (`Cmd/Ctrl+T`) + "agent with flags" UI via `/impeccable` —
  **awaiting the user's visual sign-off** (the `agent_list`/`agent_detect`/`agent_launch` IPC + the TS mirror
  are in place to wire it). Then **E5** (5-state idle FSM + activity surfacing). E6 `later`; E7 in P9. Branch
  **not pushed / no PR** — awaiting the user's call. `package-lock.json` left untracked (recorded decision).
...
**A. (Phase 7, code) Build the E4 launch picker UI, then E5.** Baseline: branch `feat/phase-7-agent-launch`,
newest `a7235c6`; gate **Rust core 177 / store 15 / sys 15 / pty 11 (+3 ignored) / UI 60**. **E4 backend + E8
are code-complete** on this branch (see the top Decisions entry): `Facade::launch_agent` runs a tool as an
interactive-PTY Agent in the project dir with the env passed through (no credential injected); the
`agent_list`/`agent_detect`/`agent_launch` Tauri commands + the `AgentTool`/`AgentKind`/`DetectedTool` TS
mirror are in place. **Next slice = the E4 launch picker UI** (`Cmd/Ctrl+T` + the "agent with flags" affordance,
surfacing detected/installed tools) **via `/impeccable`** — **confirm the visual specifics with the user first
(it's a new surface; DESIGN.md is the source of truth)**; wire it to `agentList`/`agentDetect`/`agentLaunch`,
add a UI test, and the headless backend test already proves the launch. After E4, **E5** (the 5-state idle FSM
sampler `IDLE/PERMISSION/THINKING/WORKING/ERROR` + per-provider Strategy + activity surfacing); **E6**
summarization is `later`; **E7** completes in P9. The branch is **not pushed / no PR** — awaiting the user's
call. (Copilot/Kimi are seeded built-in tools; the **package-lock.json** gitignore question is still open —
see Decisions.)

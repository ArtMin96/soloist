...
`feat/phase-9-timers` (off `main`, commit `1532251`) — **PR #23 open** — gate green (core 288 / store 38 / ipc 14 /
app 30 / mcp 44 / sys 5 +10 / pty 1 +12 +3 ign / UI 78; `just lint` + `just test` + `cargo check -p soloist-app
--no-default-features` all pass). **Merge of PR #23 is the user's call — do NOT self-merge; ask.** **In order:**
1. **Review + merge PR #23 — the timers slice** (`feat/phase-9-timers`). Additive vertical on the established

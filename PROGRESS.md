...
**A. (Phase 8 session 3) Finish the remaining v1 MCP tools onto the skeleton.**
Sessions 1–2 are done. Session 1 (skeleton: F1/F3/F4 + read F5/F6) is merged to `main`. **Session 2 (this
session) landed F6 process control + F7 + F11 + F13 on branch `feat/phase-8-mcp-tools` (PR open — review/merge
it first; gate `just lint` + `just test` exit 0).** The full path **agent → `soloist-mcp` (rmcp stdio) →
IPC/UDS → app → `Facade` → core** is proven for read AND action tools, with the **F13 scope guard** (C8
`core/facade/scoped.rs`) + the C2 trust gate. **Done so far:** `whoami`/`register_agent`/`select_project`/
`list_projects`/`get_project_status`/`list_processes`/`get_process_status` (S1) + `start_process`/`stop_process`/
`restart_process`/`send_input`(+`wait_ms`)/`spawn_agent`/`list_agent_tools` (S2). **Still to add** — each a thin
`crates/mcp` handler (clean-room `schemars` params) → **one new/existing `Facade` method** → an `IpcRequest`
arm in the now-**async** `handle_request`; no domain logic in the handler; scope/trust in the core:
  1. **F8 bulk** — `start_all_commands`/`stop_all_commands`/`restart_all_commands` (trusted-in-scope only).
     Route to the existing `Supervisor::start_all`/`stop_all`/`restart_running`, scoped to the session's
     effective project. Add a scoped `Facade` wrapper next to the others in `core/facade/scoped.rs`.
  2. **F9 output** — `get_process_output` (rendered) / `get_process_raw_output` (raw) / `search_output` /
     `search_raw_output` / `clear_output` (buffer only) / `flush_terminal_perf` / `get_process_ports`. Reuse
     `Supervisor::rendered`/`rendered_tail`/`pty_scrollback` + `ProcessView::ports`; `search_*` is a new small
     core helper over the rendered/raw buffers. These are reads — decide per-tool whether scope-gated (likely
     open like the other reads, `plan/05` §12) and keep payloads bounded.
  3. **F10 services** — `services_list`, `wait_for_bound_port` → the existing **`Facade::wait_for_port`** (async
     — `handle_request` is already async after S2, so this slots in). `services_list` projects the running
     command set + ports.
  4. **F6 remainder** — `rename_process` / `close_process` / `select_process`. These need **new core behaviour**:
     a `Supervisor::rename` and a `close`/`remove` (C2 — decide close semantics: stop the group + remove from
     the registry + which `DomainEvent` fires), and a session **selected-process** in C8 `identity` (mirrors
     `select_project`). Then scoped `Facade` wrappers + tools.
Also deferred: **`spawn_process`** (an arbitrary *terminal* command over MCP — trust-sensitive, outside
`launch_agent`; design its trust treatment before adding), **F13 binding-authenticity** (peer-cred → pgid; the
read+action tools are currently bound-by-trust-of-`SOLOIST_PROCESS_ID`, `plan/05` §12), and the **F1 helper
bundling** (`externalBin`/sidecar — a **Phase-12** packaging item; don't pull forward). **F2/F12/F14 stay
`later`.** Keep slices small + gate-green; each its own commit. Use the **official MCP docs** + `rmcp` via
context7 (the `mcp-builder` skill is **not installed**). Recorded gaps: MCP tool **param schemas are ours**
(`plan/05` §12) — document each; the `ipc`-deps-`core` type-linkage may want an `ipc` split in P10 (`plan/06` §2).

...
> quiet is not a completion signal.** Gate results are **agent-reported, not independently confirmed**. Two
> owner-approved items were **in flight** when this was written — extending the preamble to the five remaining
> providers, and the live `just dev-alongside` verification — so `plan/02` **O13** may already have moved past
> what this entry records. See the entry below.
>
...
- **CHILD-AGENT LIFECYCLE — a worker reports to its lead, and may close itself — `Done — pending verify` (2026-07-31, branch `feat/child-agent-lifecycle`, 9 commits, not merged).** The owner reported that a spawned worker neither reported its result to its lead nor closed itself when done. A review of the whole `spawn_agent` path established that **neither behaviour was ever built** — no channel carried a worker's *content* to its lead (the only autonomous PTY write is a fired timer, and a timer always delivers to the process that armed it), and nothing removed a process from the registry on exit (`remove_returning_handle` has no call site in the exit path) — and found a third thing that **was** built and **was** broken: the delegation loop's "worker finished" signal is three seconds of terminal quiet, which a worker that is still booting satisfies. The review's own reproduction watched Soloist tell a lead *"all 1 watched agents are idle"* about a worker still rendering `connecting…` that had never been given a task. **The full review, with its verbatim reproductions, is tracked at `docs/research/child-agent-flow-review.md`** (it was first written under `.scratch/`, which the owner removed by decision 2026-07-27; note `CLAUDE.md` still names `.scratch/<feature>/` as the issue-tracker home and contradicts that decision). The substance is also recorded here, so this entry stands alone.
  ...
  - **Evidence — gates are AGENT-REPORTED, not independently confirmed.** Each implementing agent reported its own gate run green except the one known pre-existing red on `main` (below). **This ledger session ran no gates itself** (docs-only), and an **independent verification pass was still in flight** when this entry was written, so no count here has been confirmed by a second party. Read this as "the authors say green", not as a verified gate; if the verifier finds something material it lands as a follow-up entry. New Rust tests include `quiet_before_an_agent_has_worked_is_not_observed_as_a_finished_turn`, `reset_drops_the_turn_observed_before_the_agent_stopped`, `a_watched_worker_that_has_not_begun_a_turn_does_not_fire_the_timer`, `a_watched_process_that_exits_while_working_fires_the_timer`, `fire_when_idle_reports_the_same_state_in_the_timer_it_returns`, `a_lead_that_never_bound_still_has_its_worker_nest_under_it`, `a_caller_soloist_cannot_name_spawns_a_root`, `a_workers_report_reaches_its_lead_as_a_submitted_turn`, `a_caller_no_agent_spawned_has_no_lead_to_report_to`, `a_report_to_a_departed_lead_is_refused`, `an_oversized_report_is_refused_and_the_cap_itself_is_allowed`, `an_armed_process_is_closed_when_its_run_ends`, `an_armed_process_that_crashes_is_closed_too`, `an_unarmed_process_rests_in_the_registry_after_it_exits`, `a_lagged_reactor_still_closes_a_finished_process`, `a_refused_bind_is_reported_by_whoami`, `a_bind_that_succeeds_clears_an_earlier_refusal`, `a_spawned_worker_opens_on_its_orchestration_context`, `an_opening_turn_reaches_only_the_launch_that_asked_for_one`, `a_worker_is_told_that_finishing_quietly_signals_nothing`, and the five `initial_prompt` per-provider tests; frontend `nests a worker under a lead of another kind, in the lead's group` and `jumps to a worker nested under a lead of another kind`.
  ...
  - **Pre-existing red on `main`, flagged not fixed.** `crates/pty/tests/integration.rs:658 create_terminal_runs_an_interactive_shell_in_the_project_dir` — the one red the implementing agents reported, a load-timing flake already recorded under the DIAGRAMS and B11 entries and A/B'd against a clean tree **in earlier sessions**. It is unrelated to this branch (which adds no PTY behaviour; `a208e60`/`5c7004c` touch `crates/pty/tests/*` only to pass the new spawn arguments). An A/B on *this* branch was still outstanding when this entry was written.
  - **IN FLIGHT AS THIS ENTRY WAS WRITTEN — two owner-approved work items started this session and not finished in it.** (a) **The spawn preamble is being extended from Claude + Codex to the remaining five providers** — Amp, Gemini, OpenCode, Copilot, Kimi — each grounded in **fetched vendor documentation**, with any provider whose first-turn convention cannot be established **left without an opening turn** rather than given an invented flag (a CLI that never starts is worse than a worker that is not briefed). **This will change the O13 row again**, so a later reader should **re-check `plan/02` O13 against the code rather than trusting the text committed here**. (b) **Live end-to-end verification against a `just dev-alongside` build**, re-running the delegation reproductions from the review report — the pass that would move this work off `Done — pending verify`. Neither is reflected in the row text or the commits listed above.
  ...
  - **Next session should start with:** **picking up the two in-flight items above** — first re-read `plan/02` **O13** and the per-provider `initial_prompt` strategy against the code, since the five-provider preamble extension was mid-flight when this was written and the row may already be stale; then finish that extension (vendor docs fetched per provider, no invented flags), and run the live verification. That verification is: launching this branch against a real app with **`just dev-alongside`** (an installed Soloist is running; plain `just dev` would hijack its MCP socket and exit without opening a dev window) and walking the loop end to end — spawn a worker with `close_when_done: true`, confirm it opens on the orchestration preamble, have it call `report_to_lead`, confirm the report lands on the lead's terminal as a submitted turn and that the worker's row leaves the sidebar when its run ends; then spawn one from a Terminal lead and confirm the sidebar nests it. Then decide whether the noisy-boot idle gap needs anything further now that completion is explicit, and whether `ScopedFacade::timer_list`'s un-enriched views are worth fixing. Before any of that, confirm the `crates/pty` flake is red on `main` too.

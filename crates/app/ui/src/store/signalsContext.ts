...
// otherwise drill through three pass-through components, so it travels by context. The context now
// carries the external store (not the state itself) so each `useSignal` reader subscribes to only
// its own process's slice — a ~1 Hz tick for one process no longer re-renders every reader. The
// default is an empty store, so a component rendered without the provider (a focused test) reads an
...
export function useSignal(id: number): ProcessSignal {
  ...
  // on every tick of any process, so cache this id's derived slice keyed on the state reference and
  // its value: a tick for another process reuses the cached slice (no re-render); only a change to
  // this id's telemetry yields a new one.
  const cache = useRef<{ state: SignalState; slice: ProcessSignal } | null>(null);
  const getSnapshot = useCallback(() => {
    const state = store.getSnapshot();
    const cached = cache.current;
    if (cached && cached.state === state) return cached.slice;
    const slice: ProcessSignal = {
      metrics: state.metrics.get(id),
      attempt: state.attempts.get(id),
      activity: state.activity.get(id),
    };
    if (cached && sameSignal(cached.slice, slice)) {
      cache.current = { state, slice: cached.slice };
      return cached.slice;
    }

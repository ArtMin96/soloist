...
// affordance stays visible). A running agent that has been auto-summarized shows its latest
// one-line idle summary as a quiet caption beneath the name (opt-in; absent otherwise, so the
// row stays a single dense line). The selected row is an azure-tinted rounded fill — the macOS
...
export function ProcessRow({
  ...
  const { metrics, attempt, activity, summary } = useSignal(process.id);
  ...
  return (
    <div
      ...
      className={cn(
        "group/row relative flex cursor-default items-start gap-2 rounded-md pr-1 pl-2.5 text-[0.8125rem] outline-none",
    ...
    >
      {/* The indicator stays centered on the name line, so a two-line row keeps the dot beside
          the name rather than the middle of the stack. */}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {summary && (
          <p
            title={summary}
            className="motion-safe:animate-summary-in truncate pb-1 text-[0.6875rem] leading-tight text-muted-foreground"
          >
            {summary}
          </p>
        )}

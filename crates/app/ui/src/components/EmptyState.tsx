...
interface EmptyStateProps {
  ...
  onOpenProject: () => void;
  /** Feedback when the last opened folder declared no processes; replaces the resting copy. */
  notice?: string | null;
}
...
// action, so it carries the one azure accent. After an open that loaded nothing, `notice`
// names what happened so the action is never silent.
export function EmptyState({ hasProcesses, onOpenProject, notice }: EmptyStateProps) {
  const message = hasProcesses
    ? "Select a process in the sidebar to open its terminal."
    : (notice ?? "No project loaded. Open a folder with a solo.yml to supervise its stack.");
  return (

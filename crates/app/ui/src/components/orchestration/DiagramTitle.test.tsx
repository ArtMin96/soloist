...
function startRename(): HTMLInputElement {
  ...
  return screen.getByRole("textbox", { name: "Diagram name" }) as HTMLInputElement;
}

describe("DiagramTitle", () => {
  it("rests as the humanized title and edits the raw handle", () => {
    render(<DiagramTitle name="auth-flow" onRename={vi.fn()} />);
    expect(screen.getByRole("button").textContent).toBe("Auth flow");
    expect(startRename().value).toBe("auth-flow");
  });

  it("commits on Enter", async () => {
    const onRename = vi.fn().mockResolvedValue(undefined);
    render(<DiagramTitle name="deploy-pipeline" onRename={onRename} />);
    const field = startRename();

    fireEvent.change(field, { target: { value: "Deploy pipeline" } });
    fireEvent.keyDown(field, { key: "Enter" });

    await waitFor(() => expect(onRename).toHaveBeenCalledWith("Deploy pipeline"));
  });
  ...
  it("cancels on Escape, restoring the title and writing nothing", () => {
    ...
    expect(screen.getByRole("button").textContent).toBe("Deploy pipeline");
  });

  it("keeps the typed name and names the refusal when the core rejects it", async () => {
    const onRename = vi.fn().mockRejectedValue("a diagram named that already exists");
    render(<DiagramTitle name="deploy-pipeline" onRename={onRename} />);
    const field = startRename();

    fireEvent.change(field, { target: { value: "data-model" } });
    fireEvent.keyDown(field, { key: "Enter" });
    ...
    const after = screen.getByRole("textbox", { name: "Diagram name" });
    expect(after).toHaveProperty("value", "data-model");

  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-border p-3">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <span className="shrink-0 font-mono type-body text-muted-foreground">
          {`#${pullRequest.number}`}
        </span>
        <p className="min-w-0 flex-1 truncate type-body font-[550]">{pullRequest.title}</p>
        {pullRequest.draft && <StateBadge>{DRAFT_LABEL}</StateBadge>}
        <StateBadge>{STATE_LABEL[pullRequest.state]}</StateBadge>
      </div>
      <div className="flex items-center gap-2">
        <p className="min-w-0 flex-1 truncate font-mono type-body text-muted-foreground">
...
function StateBadge({ children }: { children: string }) {
  return (
    <Badge variant="muted" className="shrink-0 rounded-md px-2 type-label shadow-none">
      {children}
    </Badge>
  );
}

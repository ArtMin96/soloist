...
impl WatchReactor {
    ...
    /// Establishes a watch per distinct watch-eligible project root, then debounces matching
    ...
    pub async fn run(mut self) {
        ...
        // Compile each watch-eligible command's rule once, and watch each distinct project
        // root once. `handles` is held for the reactor's lifetime: dropping a handle stops
        // its OS watch (the bounded-resource contract), so they live exactly as long as the
        // watches do.

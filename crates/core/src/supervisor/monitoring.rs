...
/// A command eligible for file-watch restarts: its id, the project it belongs to, the project
/// root its globs are relative to, and its `restart_when_changed` globs. The file-watch reactor
/// reads these to know which roots to watch, what a change should restart, and which project to
/// report a refused watch against; trust is re-checked at restart.

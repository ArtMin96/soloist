...
impl CorePortsBuilder {
    ...
    /// Overrides the durable template store the coordination aggregate persists to (C6; defaults to
    /// [`NoopTemplateRepo`], which stores nothing). The real adapter is SQLite, the same store
    /// backing every other durable repository; templates are durable shared content — global or
    /// project-scoped, of any [`TemplateKind`](crate::template::TemplateKind) — that survives a
    /// restart.
    pub fn template_repo(mut self, template_repo: Arc<dyn TemplateRepo>) -> Self {
        self.ports.template_repo = template_repo;
        self
    }

...
use crate::coordination::{IdleMode, TimerStatus, MAX_TIMER_BODY_BYTES};
use crate::ids::{ProcessId, ProjectId, SessionId};
use crate::ports::{ProjectRepo, TokioClock};
use crate::testing::{
    authentic_session, terminal_registration, FakeProjectRepo, FakeSpawner, FakeTimerRepo,
    FakeTrustRepo, TEST_PEER_PGID,
};

/// A façade over in-memory fakes with the timer store wired, so the timer methods persist.
...
/// running inside that process — so the session has both an effective project and a timer owner.

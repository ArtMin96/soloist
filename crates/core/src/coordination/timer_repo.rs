...
impl StoredTimer {
    ...
    /// remainder. `waiting_on` and `already_idle` default to their empty/false sentinels; the
    /// façade enriches them from live idle state before returning to a caller.

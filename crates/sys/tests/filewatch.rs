//! Integration check against the real `notify` file watcher, over the session-based port
//! (`FileWatcher::open` / `WatchSession`): a file created, changed, removed, or renamed under a
//! registered path is reported on the change channel with the right [`FileChangeKind`], one
//! `notify` instance backs many registered directories, dropping a registration (or the whole
//! session) stops its watch, and a path the OS will not watch comes back as a refusal rather than
//! as a registration that stays quiet. The mock-clock matching/debounce behaviour is covered in
//! the core; this is where which *kinds* of filesystem event reach the core at all is pinned,
//! since that is the one decision this adapter makes. Uses real time (a short poll budget), like
//! the other OS-adapter integration tests.
...
/// Blocks until a value arrives on `rx` or `budget` elapses, returning it if seen.
fn change_within(rx: &mut mpsc::Receiver<FileChange>, budget: Duration) -> Option<FileChange> {
...
fn open_session(tx: mpsc::Sender<FileChange>) -> Arc<dyn soloist_core::filewatch::WatchSession> {
...
#[test]
fn dropping_the_session_stops_every_registration_it_held() {
    let dir = tempfile::tempdir().expect("temp dir");
    let root = dir.path().to_path_buf();
    let (tx, mut rx) = mpsc::channel(64);

    let session = open_session(tx);
    session.watch_dir(&root).expect("register the directory");
    drop(session);
    // Give the backend a moment to tear the watcher down before changing the tree.
    std::thread::sleep(Duration::from_millis(100));
    while rx.try_recv().is_ok() {}

    fs::write(root.join("after_drop.txt"), b"x").expect("write after the session was dropped");

    assert!(
        change_within(&mut rx, QUIET).is_none(),
        "no changes are reported once every registration's session is dropped",
    );
}

#[test]
fn a_file_removed_from_a_registered_directory_is_reported_as_vanished() {
    let dir = tempfile::tempdir().expect("temp dir");
    let root = dir.path().to_path_buf();
    let target = root.join("doomed.txt");
    // Created before the registration, so the removal below is the only event it can report — a
    // create for the same path cannot be mistaken for it.
    ...
    let change = change_within(&mut rx, BUDGET)
        .expect("a removal under the registered directory is reported");
    assert!(
        change.path.ends_with("doomed.txt"),
        "expected the removed file, got {:?}",
        change.path,
    );
    assert_eq!(change.kind, FileChangeKind::Vanished);
}

#[test]
fn a_file_renamed_within_a_registered_tree_reports_both_halves() {
    let dir = tempfile::tempdir().expect("temp dir");
    let root = dir.path().to_path_buf();
    ...
    session
        .watch_tree(&root)
        .expect("register the tree recursively");

    fs::rename(&before, root.join("after.txt")).expect("rename watched file");

    // The backend reports a rename's two sides as a disappearance and an appearance rather
    // than a single event, so both must reach the core (`classify` maps
    // `Modify(Name(RenameMode::From))` to `Vanished` and `..To)` to `Appeared`).
    let vanished = change_ending_with(&mut rx, "before.txt", BUDGET)
        .expect("the old name is reported as vanished");
    assert_eq!(vanished.kind, FileChangeKind::Vanished);
    let appeared = change_ending_with(&mut rx, "after.txt", BUDGET)
        .expect("the new name is reported as appeared");
    assert_eq!(appeared.kind, FileChangeKind::Appeared);
}

#[test]
fn a_change_in_a_subdirectory_of_a_recursively_registered_tree_is_reported() {
    let dir = tempfile::tempdir().expect("temp dir");
    let root = dir.path().to_path_buf();
    ...
    fs::write(nested.join("main.rs"), b"fn main() {}").expect("write nested file");

    let change =
        change_within(&mut rx, BUDGET).expect("a recursive registration reports a nested change");
    assert!(
        change.path.ends_with("main.rs"),
        "expected the nested file, got {:?}",
        change.path,
    );
}

#[test]
fn a_path_that_cannot_be_watched_says_so_rather_than_going_quiet() {
    ...
    let refused = session
        .watch_dir(&missing)
        .expect_err("a path that does not exist cannot be registered");

    // A registration that reports nothing is what an untouched tree looks like too, so one the
    // OS turns down has to come back as a refusal — otherwise the watch set above cannot tell a
    // working registration from a dead one, and the subsystem dies in silence.
    assert_eq!(
        refused,

...
/// Writes an executable shell script (ignoring its arguments, so it answers `--version`) that
/// runs `body`, and returns its absolute path.
...
fn the_version_probe_reflects_whether_a_command_runs() {
    ...
    // A short timeout so the hang case completes promptly.
    let probe = CommandVersionProbe::with_timeout(Duration::from_millis(300));
    ...
    // A missing binary is not installed.
    assert!(
